...
- **Knowledge graph export**: the result toolbar exports the complete graph as JSON (including source, chunk, evidence, verification, and audit data), or as separate node and edge CSV files. Exports always contain the full graph, independent of the active chapter filter; trajectory graphs support the same exports.
