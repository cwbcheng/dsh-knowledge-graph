...
- **《学习观》 ontology graphs (`learning-view-v1`)**: a second **extraction ontology**, not a re-cut of an existing graph. Node and relation types follow 于建国's *学习观*: **knowledge** (`concept` / `feature` / `rule` / `discrimination_model` / `connection_model`) and **learning material** (`intension_description` / `feature_description` / `positive_example` / `negative_example` / `contrast_group` / `extension_contrast` / `relation_material` / `factor_material` / `property_material` / `segment_example_group` / `verification_material` / `data_or_experience` / `memory_material`) are built as separate nodes — 18 node types and 21 relation types. Every material carries two orthogonal coordinates, `modelKind` (discrimination / connection) and `layer` (upper / lower), which is what makes knowledge-collapse diagnostics possible: lower-missing, upper-missing, layer mismatch, model mismatch, empty connection model, memorizing words, words-without-meaning, meaning-without-words, recycled verification, missing verification, and learning material stored as memorization material. Extraction is LLM-only and still yields a storable / queryable / exportable / appendable knowledge graph; documents with no ontology keep the original proposition ontology and behave exactly as before. **How to use it**: pick 《学习观》 in the "知识图模式" selector in the extraction form; the default stays the proposition ontology and is sent as no parameter at all, so existing behaviour is unchanged. A document renders under the ontology it was extracted with — the mode badge, legend, coordinate grid, diagnostics strip and edge attributes all follow — and an already-stored document keeps the proposition graph it has, so seeing the 《学习观》 version means extracting again. The mode only affects new extractions; it never rewrites a document. **Edge attributes are ontology data too**: the `role` on `maps_between` (input/output), the sign on `contrasts`, and the `mode` on `compares_*` (contrast/analogy) are appended to the edge label, with the value labels shipped by the ontology — the proposition ontology declares no attributes, so it renders none. **Layout reads the ontology too**: the layered view's backbone, satellite attraction, and branch lanes are derived from the document's own `relationTypes` `family` / `weight`, so a 《学习观》 graph opens out along its own structure instead of collapsing into a hairball because its relation names are absent from a hardcoded proposition table. Each material's two coordinates are also shown: a grid under the legend breaks the graph into 判别上料 / 判别下料 / 联结上料 / 联结下料 plus other materials and knowledge, and only a profile that declares coordinates renders it. See [`docs/learning-view-ontology.md`](docs/learning-view-ontology.md).
- **Relation-aware layered layout**: layered mode ranks and resolves overlaps inside each undirected connected component before compactly packing the component rectangles, so unrelated subgraphs can no longer stretch direct neighbours across shared global rows. Within each component, relation-weighted ordering treats the backbone relations (`causes/infers` for the proposition ontology; whatever a profile marks `family = backbone`) as the spine, keeps satellite relations as nearby branches, and uses directional relations as bounded soft proximity preferences. Which relations those are, and their weights, come from the current document's ontology rather than being hardcoded in the client. When edges share a node or inter-row band, arrow ports, horizontal channel tracks, and overlapping vertical corridors are allocated independently, with taller inter-row channels reserved for dense horizontal tracks; neighbouring relation chips are deterministically staggered within a readable distance when their rectangles collide; in pathological saturation an unplaceable chip stays hidden until its relation is hovered or selected, so lines, labels, and arrows do not crowd together. New users default to layered layout; existing saved layout preferences are preserved.
- **Connected-neighbour compaction**: wrapped rows are an initial placement, not permanent rank barriers. A bounded, deterministic two-dimensional relaxation shortens relation-weighted edges while preserving reasoning order, pinned backbone lanes, node spacing and inter-row routing channels. Empty rows are removed afterward. Corridor search uses actual obstacle intervals rather than a fixed search radius. High-degree hubs and cross-links can still require longer edges; this changes only the view, never the canonical graph or its relations.
- **Two-way linking**:
...
- **Consumption APIs are canonical-only, single-pass, and bounded**: document queries/answers load canonical state from Host memory or SQLite and fence it with `expectedRevision`. SQLite performs type/section/status/text prefiltering, all-candidate scoring, indexed from/to expansion, exact source-unit hydration, and optional fallback in one query path rather than SQLite → Host double retrieval. Lexical nodes use `(paragraph,node_id)` keyset pages and source units use paragraph keysets while retaining only a bounded top set, so late exact matches survive common early candidates without growing `OFFSET` rescans. Direct matches, expanded context, evidence projections, source text, and the aggregate context envelope all have hard budgets.
...
- **Tasks are observable, deadline-bounded, and immediately cancellable**: live progress (stage / elapsed / chars received / warnings) is surfaced and every long operation has a cancel button. Wall-clock deadlines cover stream acquisition and iteration, with distinct typed `timeout` and `cancelled` outcomes. Completed cancellation hooks are removed and every runner clears `activeTask` in `finally`, so a non-cooperative provider cannot permanently hold the foreground task slot.
...
- **Lossless per-chunk checkpoints**: every successful chunk records `nextBatchIndex`, the complete semantic state accumulated so far, and the task's frozen `baseRevision`; append also retains `baseSource / baseStaging`. Trajectory checkpoints additionally retain `traceEvents`, and trajectory append checkpoints retain `baseTraceText/baseTraceEvents`. Both `/extract` checkpoint input and Host-restart `/resume-extract` require that the canonical revision still equals that frozen base; stale checkpoints are rejected rather than rebased. Canonical task admission is reserved before asynchronous revision lookup, so concurrent `/extract` requests cannot both enter. Completed batches are not re-run. Checkpoint v2 is never truncated to the 800-node renderer budget and lives in Host/SQLite; deterministic failures stay terminal.
