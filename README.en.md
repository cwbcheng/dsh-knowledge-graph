...
- **Content-unit index = anchor**: Host and Client split and number the text with the same algorithm (blank lines stay hard boundaries; long natural paragraphs are further split at sentence boundaries, with soft-punctuation fallback for run-on sentences); the prompt requires every node to report its source unit index; the client maps units **deterministically** instead of relying on the LLM to quote verbatim.
