...
- **轨迹知识图（会话视图标签页）**：对话区新增第三个标签页「知识图」（位于 对话 / 轨迹 旁），一键把**当前会话的完整执行轨迹**（用户消息、工具调用、工具结果、AI 回复）拆成知识图——可视化这个 Agent **查到了什么事实、做出了什么推论、用了什么方法**，并在图与轨迹事件之间**双向定位**（点击节点滚动到对应事件，点击事件在图中聚焦对应节点）。
...
- 对话区顶部出现第三个标签页「知识图」（对话 / 轨迹 / 知识图），点击 → **拆解本会话轨迹**，约 15–40 秒后得到该会话的轨迹知识图。
...
5. 对话区切换到「知识图」标签页，点 **拆解本会话轨迹** 生成会话轨迹知识图；点击轨迹事件在图中聚焦节点，点击节点滚动到对应事件。
...
│   task Map; busy lock; 2h purge                │   │  会话标签页「知识图」(conversation.view)│
└────────────────────────────────────────────────┘   │    轨迹 ⇄ 知识图双向定位            │
                                                     └──────────────────────────────────────┘
```
