...
- **验证与质疑知识图**：生成图后可以检查它是否忠于原文——
  ...
  - **人工闭环**：问题按「错误 / 警告 / 建议」列出，点击问题 → 图中相关节点/边按严重度着色高亮并滚动定位原文；每条问题可**采纳修复**（改动立即应用并写入审计记录）或**忽略**；
...
- **验证以原文为唯一事实源**：快速体检在 Host 本地执行（与 Client 同一套锚点匹配算法）；深度审校按段落分批、每批只审相关子图，标准档先产生候选问题再由复核员二次过滤；无原文证据、置信度不足或目标不存在的 issue 在 Host 层直接丢弃。
- **修复不静默、可审计**：AI 只提建议，用户点「采纳」才应用补丁；每次应用写 `graph.verification.auditLog`（before/after 快照）；追加拆分后旧报告自动标记 `stale`。
