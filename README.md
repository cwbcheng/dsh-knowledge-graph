...
- **关系感知分层布局**：分层模式以 `causes/infers` 推理链作为主路径，把例子、类比、反例、定义和概念关系放成邻近分支；新用户默认使用分层布局，已有本地布局偏好保持不变。
- **双向定位**：
