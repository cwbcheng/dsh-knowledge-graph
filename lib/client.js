...
`)
      ...
      function ModelPicker({ value, onChange }) {
        ...
        const loadCatalog = useCallback(async () => {
          setLoadError(null)
          try {
            const res = await rpc('list-models', {})
      ...
      function TrajectoryTab({ sessionId }) {
        ...
        const extract = async () => {
          ...
          clearTrajResult(sessionId)
          try {
            const res = await rpc('trajectory-extract', { sessionId, ...(modelArg ? { model: modelArg } : {}) })
        ...
        const appendExtract = async () => {
          ...
          try {
            ...
            const res = await rpc('trajectory-append-extract', { sessionId, existing, ...(modelArg ? { model: modelArg } : {}) })
            if (res && res.error) { setPhase('idle'); setError(res.error); return }
            if (res && res.taskId) {
              setTaskId(res.taskId)
