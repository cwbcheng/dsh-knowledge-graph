...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function encodeVerificationRequest(payload) {
...
.kg-image-upload { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; margin: 0 0 10px; }
.kg-image-upload-btn { display: inline-flex; align-items: center; gap: 6px; }
.kg-image-upload-note { font-size: 12px; color: var(--kg-text-dim); }
.kg-image-input { position: absolute; width: 1px; height: 1px; overflow: hidden; clip: rect(0,0,0,0); white-space: nowrap; }
.kg-image-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 8px; margin: 0 0 10px; }
.kg-image-item { position: relative; display: flex; align-items: center; gap: 8px; min-width: 0; padding: 7px; border: 1px solid var(--kg-border); border-radius: 10px; background: var(--kg-panel); }
.kg-image-thumb { width: 54px; height: 54px; flex: 0 0 auto; object-fit: cover; border-radius: 7px; border: 1px solid var(--kg-border); background: rgba(127,127,127,0.08); }
.kg-image-meta { min-width: 0; flex: 1; }
.kg-image-name { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 12px; font-weight: 600; }
.kg-image-size { display: block; margin-top: 2px; font-size: 11px; color: var(--kg-text-dim); }
.kg-image-remove { border: none; background: transparent; color: var(--kg-text-dim); cursor: pointer; font-size: 16px; padding: 2px 5px; }
.kg-image-remove:hover { color: #dc2626; }
.kg-visual-note { margin: 0 0 10px; padding: 8px 10px; border-radius: 9px; border: 1px solid rgba(245,158,11,0.4); background: rgba(245,158,11,0.08); color: var(--kg-text-dim); font-size: 12px; }
.kg-source-gallery { display: flex; gap: 8px; overflow-x: auto; padding: 8px 8px 10px; border-bottom: 1px solid var(--kg-border); }
.kg-source-image { flex: 0 0 150px; border: 1px solid var(--kg-border); border-radius: 8px; padding: 6px; background: var(--kg-panel); color: var(--kg-text); cursor: pointer; text-align: left; }
@media (max-width: 640px) { .kg-win { left: 6px !important; top: 6px !important; width: calc(100vw - 12px) !important; height: calc(100dvh - 12px) !important; } .kg-win-resize { display: none; } }
.kg-source-image.on { border-color: #3b82f6; box-shadow: 0 0 0 2px rgba(59,130,246,0.22); }
.kg-source-image img { display: block; width: 100%; height: 92px; object-fit: contain; border-radius: 6px; background: rgba(127,127,127,0.07); }
.kg-source-image span { display: block; margin-top: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 11px; }
.kg-actions { display: flex; align-items: center; justify-content: flex-end; flex-wrap: wrap; gap: 10px; margin-top: 10px; }
.kg-actions .kg-image-list { flex: 1 0 100%; margin: 0; }
.kg-actions .kg-visual-note { flex: 1 0 100%; margin: 0; }
...
.kg-relation-progress { margin: 0 0 14px; padding: 12px 16px; border-bottom: 1px solid var(--kg-border); background: var(--kg-panel); }
.kg-relation-progress-head { display: flex; align-items: center; flex-wrap: wrap; gap: 8px 14px; }
.kg-relation-progress-head strong { font-size: 14px; }
.kg-relation-progress-head span { flex: 1 1 180px; min-width: 0; font-size: 12px; color: var(--kg-text-dim); overflow-wrap: anywhere; }
.kg-relation-progress details { margin-top: 6px; font-size: 12px; }
.kg-relation-progress summary { cursor: pointer; color: var(--kg-text-dim); }
.kg-relation-progress .kg-empty-sub { margin-top: 6px; }
...
.kg-gather-shell { position: relative; min-width: 0; }
.kg-gather-layer { position: absolute; inset: 0; display: flex; flex-direction: column; background: var(--kg-panel); }
.kg-gather-bar { display: flex; align-items: center; flex-wrap: wrap; gap: 6px; padding: 8px; border: 1px solid var(--kg-border); border-bottom: 0; font-size: 12px; }
.kg-gather-bar strong { overflow-wrap: anywhere; }
.kg-gather-bar select { min-width: 0; max-width: 170px; color: var(--kg-text); background: var(--kg-panel); border: 1px solid var(--kg-border); }
.kg-gather-bar button { min-width: 30px; min-height: 28px; }
.kg-gather-directions { display: inline-flex; gap: 0; }
.kg-gather-directions button { border-radius: 0; padding: 3px 6px; }
.kg-gather-directions [aria-pressed=true] { background: #dbeafe; color: #1d4ed8; }
.kg-gather-canvas { position: relative; flex: 1; min-height: 0; }
.kg-gather-canvas > .kg-graph-stage { height: 100% !important; }
.kg-gather-canvas .kg-graph { height: 100% !important; }
.kg-gather-notice { padding: 6px 8px; font-size: 12px; color: var(--kg-muted); }
.kg-gather-notice[role=alert] { color: #dc2626; }
.kg-gather-updated { position: absolute; bottom: 8px; left: 8px; right: 8px; padding: 6px 8px; background: var(--kg-panel); color: var(--kg-text); border: 1px solid var(--kg-border); font-size: 12px; }
.kg-graph-stage > .kg-load-overlay { position: absolute; inset: 0; z-index: 6; background: var(--kg-panel); }
.kg-load-content { width: min(360px, 100%); padding: 20px; text-align: center; overflow-wrap: anywhere; }
.kg-load-heading { display: flex; justify-content: center; align-items: center; gap: 10px; color: var(--kg-text); font-size: 14px; }
.kg-load-detail { margin-top: 10px; font-size: 12px; line-height: 1.6; }
.kg-load-progress { display: block; width: 100%; height: 8px; margin-top: 14px; accent-color: #3b82f6; }
.kg-document-loading { min-height: 140px; border-bottom: 1px solid var(--kg-border); }
...
.kg-ontology-badge { display: flex; align-items: baseline; gap: 8px; margin: 6px 0 0; padding: 4px 8px; border-left: 2px solid var(--kg-border); color: var(--kg-text-dim); font-size: 11px; }
.kg-ontology-badge-label { color: var(--kg-text); font-weight: 600; }
.kg-ontology-badge-counts { opacity: 0.85; }
...
`)
      ...
      function watchVerificationTask({ ctx, taskId, isCurrent, setProgress, onReport, onFinish }) {
        ...
        const tick = async () => {
          ...
          let response
          try {
            response = await rpc('task-status', { taskId })
      ...
      async function cancelVerificationTask(taskId, isCurrent, setProgress) {
        ...
        try {
          const response = await rpc('task-cancel', { taskId })
      ...
      function RelationDiscoveryStatus({ coverage }) {
        ...
        return h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', maxWidth: '100%' }, title: '候选检索覆盖，不代表所有可能关系已找到或已通过语义审校' },
          ...
          Number.isInteger(coverage.savedGroups) ? h('span', { role: 'status' }, (coverage.durable ? '候选已落盘 ' : '候选暂存 ') + coverage.savedGroups + '/' + coverage.totalGroups + ' 组 · ' + coverage.savedTargets + ' 节点 · 尚待审校') : null,
      ...
      function TaskPauseControls({ taskId, kind = 'extract', status, progress, onResumed, onChanged, resumeModel, resumeModelSwitch }) {
        ...
        const act = async () => {
          ...
          try {
            const res = paused
              ? kind === 'verify'
                ? await rpc('resume-verify', { runId: taskId, resumePaused: true,
                  ...(resumeModel ? { model: resumeModel } : {}) })
                : await rpc('resume-extract', { runId: taskId, resumePaused: true })
              : await rpc('task-pause', { taskId })
      ...
      function BackgroundTaskProgress({ ctx, knownTaskIds = [], busyError, onOpenDocument, onStopTask }) {
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            try {
              const response = await rpc('task-active', { taskId: observedId.current })
              if (disposed) return
              if (!response || response.error || typeof response.busy !== 'boolean') throw new Error('unsupported')
              const task = response.task || response.trackedTask
              if (task) observedId.current = knownTaskIds.includes(task.taskId) ? '' : task.taskId
              setSnapshot(previous => ({ busy: response.busy, task: task || (observedId.current && previous?.task ? { ...previous.task, status: 'not_found', progress: null, error: { message: '任务已不在当前服务中；已保存知识图不受影响，不会自动重新提交。' } } : null) }))
              setConnectionError('')
              failures = 0
              stop = ctx.timeout(tick, response.busy ? 3000 : 10000)
            } catch (error) {
              if (disposed) return
              failures += 1
              setConnectionError('后台任务状态暂时不可用，正在重连；旧版服务需更新后才能查询。')
              stop = ctx.timeout(tick, Math.min(3000 * Math.pow(1.5, failures), 30000))
            }
          }
          tick()
          return () => { disposed = true; if (stop) stop() }
      ...
      function forgetPendingTask(taskId) {
        try {
          const saved = JSON.parse(localStorage.getItem(LS_PENDING) || 'null')
          if (saved?.taskId === taskId) localStorage.removeItem(LS_PENDING)
        } catch (error) {}
      }
      // --------------------------- constants ----------------------------
      ...
      async function mergeServerHistory(list) {
        try {
          // `rpc('document-list')` is rewritten to `rpc('document-list')`
          ...
          const res = await rpc('document-list')
      ...
      // segment that owns the label, then nearby two-dimensional slots. The
      // caller owns `occupied` and must recreate it for each immutable layout
      ...
      function placeLayeredEdgeLabel(x, y, width, height, occupied, nodeRects, edgeIndex, axis) {
        ...
        // At pathological densities, hiding one chip is less ambiguous than
        // placing it on top of another chip or hundreds of pixels from its edge.
      ...
      function layoutLayered(nodes, edges, sizes, pinnedXOut) {
        ...
        const compactRows = () => {
          ...
          for (let pass = 0; pass < 12; pass++) {
            ...
            for (const node of ordered) {
              ...
              // Merge the two already ordered sides of meanY lazily. Most
              // searches stop after a few rows; do not sort every possible row.
              let left = Math.max(min - 1, Math.min(max, Math.floor(meanY / LAYER_Y_GAP)))
              let right = left + 1
              while (left >= min || right <= max) {
                const row = right > max || (left >= min &&
                  Math.abs(left * LAYER_Y_GAP - meanY) <= Math.abs(right * LAYER_Y_GAP - meanY)) ? left-- : right++
        ...
        // Strong local semantic relations should look local too. This pass is
      ...
      function resolveLayeredOverlaps(nodes, sizes, pos, gap, pinnedX) {
        ...
        // y never changes here. Enumerate only vertically intersecting pairs,
        // but retain the original i/j order (including coincident-node ties).
        // Dense rows fall back to scanning so the cache stays linear in n.
        const ordered = entries.map((entry, index) => ({ ...entry, index })).filter((entry) => entry.size)
          .sort((a, b) => a.point.y - b.point.y || a.index - b.index)
        let maxHeight = 0
        for (const entry of ordered) maxHeight = Math.max(maxHeight, entry.size.h)
        let pairs = Array.from({ length: n }, () => [])
        let pairCount = 0
        scan: for (let i = 0; i < ordered.length; i++) {
          const a = ordered[i]
          const reach = (a.size.h + maxHeight) / 2 + gap
          for (let j = i + 1; j < ordered.length; j++) {
            const b = ordered[j], dy = b.point.y - a.point.y
            if (dy > reach) break
            if (dy > (a.size.h + b.size.h) / 2 + gap) continue
            if (++pairCount > n * 32) { pairs = null; break scan }
            pairs[Math.min(a.index, b.index)].push(Math.max(a.index, b.index))
          }
        }
        if (pairs) for (const list of pairs) list.sort((a, b) => a - b)
        for (let iter = 0; iter < 120; iter++) {
          let moved = 0
          for (let i = 0; i < n; i++) {
            const { point: a, size: sa, pinned: aPinned } = entries[i]
            if (!sa) continue
            const candidates = pairs && pairs[i]
            const count = candidates ? candidates.length : n - i - 1
            for (let k = 0; k < count; k++) {
              const j = candidates ? candidates[k] : i + k + 1
      ...
      function layeredOrthoPath(edge, a, b, sizes, pos, nodes, lane) {
        ...
        if (r1 === r2) {
          ...
          return { d, lblX: (ax + bx) / 2, lblY, labelAxis: 'x' }
        }
        ...
        return {
          ...
          lblX: xce + 14,
          lblY: (ch1e + ch2e) / 2,
          labelAxis: Math.abs(ch1e - ch2e) > 1 ? 'y' : 'x',
        }
      }
      ...
      function useGraphDocumentLoading() {
        ...
        const load = useCallback(async body => {
          ...
          try {
            ...
            const loaded = await rpc('document-load', body)
      ...
      // Hover/focus is local to one edge. Lifting it into GraphScene rebuilds
      // every SVG path and label whenever the pointer crosses a relationship.
      function GraphEdgeInteraction({ render }) {
        const [hover, setHover] = useState(false)
        const [focused, setFocused] = useState(false)
        return render(hover || focused, {
      ...
      function KnowledgeConsumePanel({ ctx, graph, sourceText, mode, model, onLocateReference, onNotify }) {
        const documentId = documentIdOfGraph(graph)
        ...
        const identityPayload = () => documentId
          ...
          : { graph, text: typeof sourceText === 'string' ? sourceText : '' }
        const stopAskTimer = () => {
          if (typeof askTimerRef.current === 'function') {
            try { askTimerRef.current() } catch (e) {}
          }
        ...
        useEffect(() => {
          ...
          stopAskTimer()
          const taskId = askTaskRef.current
          if (taskId) rpc('task-cancel', { taskId }).catch(() => {})
        ...
        const runSearch = async (overrideQuery) => {
          ...
          try {
            const response = await rpc('graph-query', {
        ...
        const locate = async (reference) => {
          if (typeof onLocateReference !== 'function') return
          try {
            const ok = await onLocateReference(reference)
            if (ok !== false) notify(reference && reference.nodeId ? '已定位知识节点与原文证据' : '已定位原文证据')
          } catch (error) {
        ...
        const scheduleAskPoll = (taskId, generation, startedAt, delay) => {
          stopAskTimer()
          askTimerRef.current = ctx.timeout(async () => {
            if (generation !== askGenerationRef.current || scopeRef.current !== scopeKey) return
            let response
            try {
              response = await rpc('task-status', { taskId })
            } catch (error) {
              if (generation !== askGenerationRef.current) return
              setAskState({ phase: 'error', taskId, progress: null, result: null, error: error && error.message ? error.message : '问答状态读取失败' })
              askTaskRef.current = null
              return
            }
            if (generation !== askGenerationRef.current || scopeRef.current !== scopeKey) return
            if (response && response.status === 'running') {
              setAskState((previous) => ({ ...previous, phase: 'running', taskId, progress: response.progress || previous.progress, error: null }))
              const elapsed = Date.now() - startedAt
              scheduleAskPoll(taskId, generation, startedAt, elapsed > 60000 ? Math.min(15000, delay + 2000) : 3000)
              return
            }
            askTaskRef.current = null
            stopAskTimer()
            if (response && response.status === 'succeeded' && response.result) {
              const result = response.result
              if ((result.documentId && documentId && result.documentId !== documentId) || (Number.isInteger(result.revision) && result.revision !== revision)) {
                setAskState({ phase: 'expired', taskId: null, progress: null, result: null, error: '知识图版本已变化，这次回答已过期，请重新提问。' })
                return
              }
              setAskState({ phase: 'done', taskId: null, progress: null, result, error: null })
              return
            }
            if (response && response.status === 'cancelled') {
              setAskState({ phase: 'cancelled', taskId: null, progress: null, result: null, error: null })
              return
            }
            if (response && response.status === 'not_found') {
              setAskState({ phase: 'expired', taskId: null, progress: null, result: null, error: '问答任务已过期，请重新提问。' })
              return
            }
            const message = response && response.error && response.error.message ? response.error.message : '知识图证据问答失败'
            setAskState({ phase: 'error', taskId: null, progress: null, result: null, error: message })
          }, delay)
        }
        const startAsk = async (overrideQuestion) => {
          ...
          stopAskTimer()
          setAskState({ phase: 'submitting', taskId: null, progress: { stage: '正在提交证据问答…' }, result: null, error: null })
          try {
            const response = await rpc('answer-graph', {
              ...
              limit: 12, hops: 1, maxNodes: 60, maxEdges: 180,
              ...(model ? { model } : {}),
            })
            if (generation !== askGenerationRef.current || scopeRef.current !== scopeKey) return
            if (!response || response.error || !response.taskId) throw new Error(response && response.error && response.error.message ? response.error.message : '无法启动证据问答')
            askTaskRef.current = response.taskId
            setAskState({ phase: 'running', taskId: response.taskId, progress: { stage: '正在检索知识与原文证据…' }, result: null, error: null })
            scheduleAskPoll(response.taskId, generation, Date.now(), 300)
          } catch (error) {
            if (generation !== askGenerationRef.current) return
            setAskState({ phase: 'error', taskId: null, progress: null, result: null, error: error && error.message ? error.message : '无法启动证据问答' })
          }
        }
        const cancelAsk = async () => {
          const taskId = askTaskRef.current || askState.taskId
          askGenerationRef.current += 1
          stopAskTimer()
          askTaskRef.current = null
          if (taskId) {
            try { await rpc('task-cancel', { taskId }) } catch (e) {}
          }
          setAskState({ phase: 'cancelled', taskId: null, progress: null, result: null, error: null })
        }
        const clearSearch = () => {
        ...
        return h('section', { className: 'kg-card', 'aria-label': '使用这张知识图' },
          h('div', { className: 'kg-consume-head' },
            h('div', null,
              ...
              h('div', { className: 'kg-hint' }, mode === 'trajectory' ? '检索轨迹知识并用可回链证据回答问题。' : '检索 canonical knowledge graph，并基于原文证据获得回答。'),
            ),
            h('div', { className: 'kg-consume-tabs', role: 'tablist', 'aria-label': '知识图消费方式' },
              h('button', { type: 'button', role: 'tab', 'aria-selected': tab === 'search', className: 'kg-consume-tab' + (tab === 'search' ? ' on' : ''), onClick: () => setTab('search') }, '结构化检索'),
              h('button', { type: 'button', role: 'tab', 'aria-selected': tab === 'answer', className: 'kg-consume-tab' + (tab === 'answer' ? ' on' : ''), onClick: () => setTab('answer') }, '证据问答'),
            ),
          ),
          h('div', { className: 'kg-consume-form' },
            tab === 'search'
              ? h('input', {
                  className: 'kg-question-input', value: query, maxLength: 600,
                  placeholder: '搜索概念、结论、规则、例子或节点 ID…', 'aria-label': '知识图检索关键词',
            ...
            h('select', { className: 'kg-consume-select', value: typeFilter, 'aria-label': '节点类型筛选', onChange: (e) => setTypeFilter(e.target.value) },
            ...
            h('select', { className: 'kg-consume-select', value: groundingFilter, 'aria-label': '原文可回链状态筛选', onChange: (e) => setGroundingFilter(e.target.value) },
              h('option', { value: 'all' }, '全部证据状态'),
              h('option', { value: 'grounded' }, '可回链原文'),
              h('option', { value: 'candidate' }, '候选证据'),
              h('option', { value: 'unsupported' }, '证据不支持'),
            ),
            h('select', { className: 'kg-consume-select', value: entailmentFilter, 'aria-label': '语义验证状态筛选', onChange: (e) => setEntailmentFilter(e.target.value) },
              h('option', { value: 'all' }, '全部语义状态'),
              h('option', { value: 'verified' }, '语义已验证'),
              h('option', { value: 'unverified' }, '语义未验证'),
              h('option', { value: 'uncertain' }, '语义不确定'),
              h('option', { value: 'unsupported' }, '语义不支持'),
            ),
            tab === 'search'
          ...
          tab === 'search'
            ? h(React.Fragment, null,
                ...
                searchState.phase === 'loading' ? h('div', { className: 'kg-consume-state', role: 'status', 'aria-live': 'polite' }, '正在查询 canonical knowledge graph…') : null,
            ...
            : h(React.Fragment, null,
                askState.phase === 'idle' ? h('div', { className: 'kg-consume-state' }, '回答只使用检索到的 canonical 节点、关系和已认证原文证据，不会把“可回链”误称为“已外部证实”。') : null,
                askState.phase === 'submitting' || askState.phase === 'running'
                  ? h('div', { className: 'kg-consume-state', role: 'status', 'aria-live': 'polite' },
                      (askState.progress && askState.progress.stage) || '正在生成证据回答…',
                      askState.progress && askState.progress.model ? ' · ' + askState.progress.model.provider + '/' + askState.progress.model.model : '',
                      h('button', { type: 'button', className: 'kg-secondary', style: { marginLeft: 8 }, onClick: cancelAsk }, '取消'),
                    )
                  : null,
                askState.phase === 'error' || askState.phase === 'expired'
                  ? h('div', { className: 'kg-consume-state kg-consume-error', role: 'alert' }, askState.error || '证据问答失败')
                  : null,
                askState.phase === 'cancelled' ? h('div', { className: 'kg-consume-state' }, '已取消本次证据问答。') : null,
      ...
      function imageBytesLabel(bytes) {
      ...
      async function preparePdfFileClient(file) {
        if (!file) throw new Error('请选择 PDF 文件')
        const name = String(file.name || '文档.pdf').slice(0, 160)
        const isPdf = file.type === 'application/pdf' || name.toLowerCase().endsWith('.pdf')
        if (!isPdf) throw new Error('仅支持 PDF 文件')
        if (!Number.isFinite(file.size) || file.size <= 0) throw new Error('PDF 为空或无法读取')
        if (file.size > MAX_PDF_INPUT_BYTES) throw new Error('单个 PDF 不能超过 ' + Math.floor(MAX_PDF_INPUT_BYTES / 1024 / 1024) + ' MiB')
        const bytes = new Uint8Array(await fileArrayBuffer(file))
        if (bytes.length !== file.size || bytes.length > MAX_PDF_INPUT_BYTES) throw new Error('PDF 读取后的大小无效')
        if (bytes.length < 5 || String.fromCharCode(...bytes.subarray(0, 5)) !== '%PDF-') throw new Error('文件内容不是有效的 PDF')
        return { name, mediaType: 'application/pdf', bytes: bytes.length, data: bytesToBase64(bytes) }
      }
      async function prepareImageFileClient(file) {
        if (!file || !IMAGE_MEDIA_TYPES.has(file.type)) throw new Error('仅支持 PNG、JPEG、WebP、GIF 图片')
        if (!Number.isFinite(file.size) || file.size <= 0) throw new Error('图片为空或无法读取')
        if (file.size > MAX_IMAGE_INPUT_BYTES) throw new Error('单张图片不能超过 ' + Math.floor(MAX_IMAGE_INPUT_BYTES / 1024 / 1024) + ' MiB')
        const bytes = new Uint8Array(await fileArrayBuffer(file))
        if (bytes.length !== file.size || bytes.length > MAX_IMAGE_INPUT_BYTES) throw new Error('图片读取后的大小无效')
        return {
          id: 'upload-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2),
      ...
      // ---- ontology choice (which ontology a NEW extraction is generated with) --
      // The catalogue comes from the host, so the picker can never offer an
      // ontology the engine cannot extract with.
      const LS_ONTOLOGY = 'dsh-kg-ontology-v1'
      let ontologyCatalogCache = null
      let ontologyCatalogLoading = false
      const ontologyCatalogListeners = new Set()
      function notifyOntologyCatalog() {
        for (const cb of ontologyCatalogListeners) { try { cb() } catch (e) {} }
      }
      function loadOntologyCatalog() {
        if (ontologyCatalogCache || ontologyCatalogLoading) return
        ontologyCatalogLoading = true
        rpc('ontology-list', {})
          .then((res) => {
            ontologyCatalogCache = Array.isArray(res && res.ontologies) && res.ontologies.length > 0
              ? { ontologies: res.ontologies, error: null }
              : { ontologies: [], error: (res && res.error && res.error.message) || '无法读取本体列表' }
          })
          .catch(() => { ontologyCatalogCache = { ontologies: [], error: '读取本体列表失败，仍可按默认本体抽取' } })
          .finally(() => { ontologyCatalogLoading = false; notifyOntologyCatalog() })
      }
      function useOntologyCatalog() {
        const snap = useSyncExternalStore(
          (cb) => { ontologyCatalogListeners.add(cb); return () => ontologyCatalogListeners.delete(cb) },
          () => ontologyCatalogCache,
          () => ontologyCatalogCache,
        )
        useEffect(() => { loadOntologyCatalog() }, [])
        return snap
      }
      function readOntologyChoice() {
        try {
          const id = localStorage.getItem(LS_ONTOLOGY)
          return typeof id === 'string' ? id : ''
        } catch (e) {}
        return ''
      }
      function writeOntologyChoice(id) {
        try { localStorage.setItem(LS_ONTOLOGY, id) } catch (e) {}
      }
      // The default ontology is sent as no value at all, so an extraction started
      // before this picker existed carries the same payload it always did.
      function ontologyPayloadArg(choice, catalog) {
        if (typeof choice !== 'string' || !choice) return null
        const record = catalog && Array.isArray(catalog.ontologies) ? catalog.ontologies.find((item) => item && item.id === choice) : null
      ...
      function SourceFigure({ image, documentId, revision, onOpen, large }) {
        ...
        useEffect(() => {
          ...
          setError(''); setUrl('')
          loadCanonicalImageUrlClient(image, documentId, revision, args => rpc('image-load', args)).then(nextUrl => {
      ...
      function ModelPicker({ value, onChange, requiresImage }) {
        ...
        if (chosen && !flat.some((opt) => opt.key === value)) {
          flat.push({ key: value, name: chosen.provider + ' · ' + chosen.model + '（已保存，图像能力未知）', imageSupport: null })
        }
        ...
        return h('span', { className: 'kg-model-picker', title: loadError || (requiresImage ? '图片需要多模态模型；“仅文本”选项已禁用，能力未知的模型将由 Host 验证' : '选择拆分、追加、AI 审校、质疑与外部核查使用的模型') },
      ...
      function WorkbenchBody({ ctx }) {
        ...
        const [gatherProjection, setGatherProjection] = useState(null)
        ...
        const [ontologyChoice, setOntologyChoice] = useState(readOntologyChoice)
        const modelCatalog = useModelCatalog()
        const ontologyCatalog = useOntologyCatalog()
        const handleOntologyChange = (id) => {
          setOntologyChoice(id)
          writeOntologyChoice(id)
        }
        const ontologyArg = ontologyPayloadArg(ontologyChoice, ontologyCatalog)
        const ontologyOptions = ontologyCatalog && Array.isArray(ontologyCatalog.ontologies) ? ontologyCatalog.ontologies : []
        const defaultOntologyId = (ontologyOptions.find((item) => item && item.isDefault) || {}).id || ''
        const shownOntology = ontologyOptions.find((item) => item && item.id === (ontologyChoice || defaultOntologyId)) || null
        ...
        const effectiveModelKey = modelKeyOf(effectiveModelArg)
        let effectiveModelImageSupport = null
        if (modelCatalog && Array.isArray(modelCatalog.providers) && effectiveModelKey) {
          for (const provider of modelCatalog.providers) {
            for (const model of provider.models || []) {
              if (provider.id + MODEL_KEY_SEP + model.id !== effectiveModelKey) continue
              effectiveModelImageSupport = Array.isArray(model.inputModalities) ? model.inputModalities.includes('image') : null
            }
          }
        }
        ...
        useEffect(() => {
          let disposed = false
          rpc('extraction-run-list').then(res => {
            if (disposed) return
        ...
        const recoverSavedRun = async (run) => {
          ...
          try {
            if (run.taskKind === 'verify') {
              ...
              if (!loaded) return
              const status = await rpc('task-status', { taskId: run.runId })
              ...
              if (!['running', 'pausing', 'paused'].includes(status?.status)) {
                ...
                res = await rpc('resume-verify', { runId: run.runId, resumePaused: true,
            ...
            const res = await rpc('resume-extract', { runId: run.runId, retryFailed: true, resumePaused: true, concurrency: extractionConcurrency, ...(effectiveModelArg ? {model: effectiveModelArg} : {}) })
        ...
        const deleteSavedRun = async (run) => {
          ...
          setRunsError('')
          try {
            const res = await rpc('extraction-run-delete', { runId: run.runId, expectedUpdatedAt: run.updatedAt })
        ...
        useEffect(() => {
          ...
          sourceImageUrlRef.current = {}
          setSourceImageUrls({})
          const graph = resultView && resultView.graph
          const source = graph && graph.source
          const visual = source && source.visualSource
          const images = visual && Array.isArray(visual.images) ? visual.images : []
          if (visual?.kind === 'markdown-assets') return () => { disposed = true }
          const documentId = source && typeof source.documentId === 'string' ? source.documentId : ''
          const revision = graph && Number.isInteger(graph.revision) ? graph.revision : (source && Number.isInteger(source.revision) ? source.revision : null)
          if (!documentId || images.length === 0) return () => { disposed = true }
          ;(async () => {
            const next = {}
            for (const image of images) {
              if (disposed || !image || !image.id) break
              try {
                const url = await loadCanonicalImageUrlClient(image, documentId, revision, args => rpc('image-load', args))
            ...
            sourceImageUrlRef.current = next
            setSourceImageUrls(next)
          })()
          return () => {
            disposed = true
            for (const url of Object.values(sourceImageUrlRef.current)) { try { URL.revokeObjectURL(url) } catch (error) {} }
        ...
        }, [resultView && resultView.graph && resultView.graph.source && resultView.graph.source.visualSource ? JSON.stringify((resultView.graph.source.visualSource.images || []).map((image) => [image.id, image.attachment && image.attachment.attachmentId])) : '', resultView && resultView.graph ? resultView.graph.revision : null])
        ...
        useEffect(() => {
          ...
          // Merge any SQLite-persisted documents into the history panel so a
          // document imported directly into the store is visible/loadable.
          mergeServerHistory(loadHistory()).then((merged) => {
            if (!disposed && merged && merged.length > 0) setHistory(merged)
          })
          ;(async () => {
            let pending = null
            let saved = null
            ...
            const restoreDocument = async (documentId, savedTitle) => {
              try {
                ...
                if (disposed || !loaded || loaded.error || !loaded.graph || !Array.isArray(loaded.graph.nodes)) return false
                ...
                setTitle(typeof savedTitle === 'string' && savedTitle ? savedTitle : ((g.source && g.source.title) || ''))
                setText(src)
                setFullText(src)
                ...
                const match = history.find((entry) => entry && entry.documentId === documentId)
        ...
        useEffect(() => {
          ...
          ;(async () => {
            try {
              const res = await rpc('document-import', { sessionId: docReq.sessionId, pending: docReq.pendingText })
              if (disposed) return
              docStore.clear()
         ...
         useEffect(() => {
           ...
           ;(async () => {
             try {
               ...
               const res = await rpc('candidate-list', {
         ...
         const loadGraphWindow = async ({ page, query, nodeLimit, force = false } = {}) => {
           ...
           const limit = Number.isInteger(nodeLimit) && nodeLimit > 0 ? Math.min(2000, nodeLimit) : currentMeta.nodeLimit
           const pageCount = Math.max(1, Math.ceil(currentMeta.totalNodes / limit))
           ...
           if (!force && !queryText && currentMeta.kind === 'window' && currentMeta.nodeOffset === nodeOffset && currentMeta.nodeLimit === limit) {
             setGraphPageDraft(String(targetPage))
             return true
           }
           if (!force && queryText && currentMeta.kind === 'query' && currentMeta.query === queryText && currentMeta.nodeLimit === limit) {
             return true
           }
           ...
           try {
             ...
             const loaded = await loadGraphDocument({
               documentId,
               nodeOffset,
               nodeLimit: limit,
             ...
             if (currentResultRef.current !== openingView) return false
         ...
         const refreshSavedGraph = () => {
           const meta = resultView && resultView.graph ? graphViewMetadata(resultView.graph) : null
           if (!meta) return
           if (meta.kind === 'all') { openAllNodes(true); return }
           loadGraphWindow({
             page: meta.page,
             query: meta.kind === 'query' ? meta.query : '',
             nodeLimit: meta.nodeLimit,
             force: true,
           })
         }
         const openAllNodes = async (refresh = false) => {
           if (!resultView || (!refresh && resultView.graph.view?.kind === 'all') || allNodesLoading || graphWindowLoading) return
           const openingView = resultView
           const documentId = documentIdOfGraph(resultView.graph)
           ...
           try {
             await graphCommitQueueRef.current.catch(() => {})
             if (currentResultRef.current !== openingView) throw new Error('知识图视图已变化，请在当前视图重新选择全部节点')
             const expectedRevision = graphRevisionRef.current
             const response = await rpc('document-export', { documentId })
             if (response?.error || !Array.isArray(response?.graph?.nodes) || !Array.isArray(response.graph.edges)) {
               throw new Error(response?.error?.message || '读取完整知识图失败')
             }
             if (currentResultRef.current !== openingView) throw new Error('知识图视图已变化，请在当前视图重新选择全部节点')
             if (response.documentId !== documentId || !Number.isSafeInteger(response.revision) ||
                 (!refresh && (response.revision !== expectedRevision || graphRevisionRef.current !== expectedRevision))) {
               throw new Error('知识图版本已更新，请重新载入后查看全部节点')
             }
             ...
             setGraphPageDraft('1')
             setGraphQueryDraft('')
             changeLayoutMode('overview')
             setResultView(makeView(asAllNodesGraph({ ...response.graph,
               graphOntology: resultView.graph.graphOntology || response.graph.graphOntology }), resultView.sourceText))
           } catch (error) {
         ...
         const resumeLostTask = async () => {
           ...
           if (pending.relationRetry === true) {
             if (pending.documentId) await loadHistoryEntry({ documentId: pending.documentId, title: pending.title || '' })
             setError({ code: 'relation_task_interrupted', message: '关系检索任务已不在当前服务中。已保存知识图不受影响，可从已保存的覆盖位置继续检索；不会自动重新提交任务。' })
             return false
           }
           try {
             const resumed = await rpc('resume-extract', {
          ...
          const tick = async () => {
            ...
            if (res && res.status === 'succeeded') {
              ...
              if (g && Array.isArray(g.nodes)) {
                ...
                if (Array.isArray(sub.images) && sub.images.length > 0) clearImageInputs()
                setSelectedNodeId(null)
                setSelectedEdgeId(null)
                setActivePara(-1)
                setPhase('done')
                ...
                if (sub.relationRetry === true) {
                  ...
                  setAppendCount(0)
                  if (Array.isArray(sub.images) && sub.images.length > 0) toastStore.show('已从 ' + sub.images.length + ' 张图片识别文字、图示和表格并生成知识图')
                }
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            ...
            let res = null
            try {
              res = await rpc('task-status', { taskId: questionTaskId })
            } catch (e) {
              if (disposed || myGen !== verifyGenRef.current) return
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            ...
            let res = null
            try {
              res = await rpc('task-status', { taskId: factTaskId })
            } catch (e) {
              if (disposed || myGen !== factGenRef.current) return
        ...
        const importMarkdownFolder = async event => {
          const files = Array.from(event.target.files || [])
          event.target.value = ''
          if (!files.length || folderPreparing) return
          setFolderPreparing(true); setError(null)
          try {
            const documents = files.filter(file => /\.md$/i.test(file.name))
            if (documents.length !== 1) throw new Error('请选择包含一个 Markdown 正文的目录')
            const md = documents[0]
            if (md.size > 4 * 1024 * 1024) throw new Error('Markdown 文件超过 4 MiB')
            const root = md.webkitRelativePath.slice(0, md.webkitRelativePath.lastIndexOf('/') + 1)
            const pictures = files.filter(file => file.webkitRelativePath.startsWith(root) && /\.(png|jpe?g|webp|gif)$/i.test(file.name))
            if (pictures.length > 128 || pictures.reduce((sum,file) => sum + file.size, 0) > 16 * 1024 * 1024) throw new Error('目录最多支持 128 张图片、合计 16 MiB')
            const uploads = []
            for (const file of pictures) {
              const data = await new Promise((resolve,reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(',')[1]); reader.onerror = () => reject(new Error('读取图片失败：' + file.name)); reader.readAsDataURL(file) })
              const ext = file.name.split('.').pop().toLowerCase()
              uploads.push({ path: file.webkitRelativePath.slice(root.length), mediaType: ext === 'jpg' || ext === 'jpeg' ? 'image/jpeg' : 'image/' + ext, data })
            }
            const response = await rpc('markdown-import', { text: await md.text(), files: uploads })
            if (response.error) throw new Error(response.error.message)
            resetAll()
            setTitle(md.name.replace(/\.md$/i, '').slice(0,200)); setText(response.text); setFullText(response.text)
            setMarkdownBundle(response)
          } catch (error) { setError({ message: error.message }) }
          finally { setFolderPreparing(false) }
        }

        const importPdfFile = async (event) => {
          const file = event && event.target && event.target.files ? event.target.files[0] : null
          if (event && event.target) event.target.value = ''
          if (!file || pdfPreparingRef.current) return
          if (resultView) { setError({ message: '当前版本仅支持用 PDF 新建知识图；追加请先提取为文字资料' }); return }
          pdfPreparingRef.current = true
          setPdfPreparing(true)
          setError(null)
          try {
            const prepared = await preparePdfFileClient(file)
            const res = await rpc('document-import', { uploads: [{ name: prepared.name, mediaType: prepared.mediaType, data: prepared.data }] })
            if (res && res.error) {
              const warnings = Array.isArray(res.warnings) && res.warnings.length > 0 ? '（' + res.warnings.join('；') + '）' : ''
              setError({ code: res.error.code, message: (res.error.message || 'PDF 没有可用正文') + warnings })
              return
            }
            if (!res || !res.text || !res.text.trim()) { setError({ message: 'PDF 没有可用正文' }); return }
            const warnings = Array.isArray(res.warnings) ? res.warnings : []
            resetAll()
            setTitle(typeof res.title === 'string' ? res.title : prepared.name.replace(/\.[^.]+$/, ''))
            setText(res.text)
            setFullText(res.text)
            toastStore.show('已读取 PDF（' + imageBytesLabel(prepared.bytes) + '），开始生成知识图…')
            await submit(res.text, typeof res.title === 'string' ? res.title : '', res.manifest && typeof res.manifest.documentId === 'string' ? res.manifest.documentId : '')
            if (warnings.length > 0) setError({ message: warnings.join('；') })
          } catch (error) {
            setError({ message: error && error.message ? error.message : '读取 PDF 失败' })
          } finally {
        ...
        const addImageFiles = async (event) => {
          const files = Array.from(event && event.target && event.target.files ? event.target.files : [])
          if (event && event.target) event.target.value = ''
          if (files.length === 0 || imagePreparingRef.current) return
          if (resultView) { setError({ message: '当前版本仅支持用图片新建知识图；追加请先使用文字资料' }); return }
          imagePreparingRef.current = true
          const baseImages = imageInputsRef.current
          const remaining = MAX_IMAGE_INPUTS - baseImages.length
          if (remaining <= 0) { imagePreparingRef.current = false; setError({ message: '一次最多上传 ' + MAX_IMAGE_INPUTS + ' 张图片' }); return }
          setImagePreparing(true)
          setError(null)
          const added = []
          try {
            let total = baseImages.reduce((sum, image) => sum + (Number(image.bytes) || 0), 0)
            for (const file of files.slice(0, remaining)) {
              const prepared = await prepareImageFileClient(file)
              if (total + prepared.bytes > MAX_IMAGE_TOTAL_BYTES) {
                URL.revokeObjectURL(prepared.previewUrl)
                throw new Error('图片总大小不能超过 ' + Math.floor(MAX_IMAGE_TOTAL_BYTES / 1024 / 1024) + ' MiB')
              }
              total += prepared.bytes
              uploadPreviewUrlsRef.current.add(prepared.previewUrl)
              added.push(prepared)
            }
            if (files.length > remaining) toastStore.show('一次最多保留 ' + MAX_IMAGE_INPUTS + ' 张图片，其余文件未加入')
            setImageInputs((current) => {
              let currentTotal = current.reduce((sum, image) => sum + (Number(image.bytes) || 0), 0)
              const kept = []
              for (const image of added) {
                if (current.length + kept.length >= MAX_IMAGE_INPUTS || currentTotal + image.bytes > MAX_IMAGE_TOTAL_BYTES) {
                  uploadPreviewUrlsRef.current.delete(image.previewUrl)
                  try { URL.revokeObjectURL(image.previewUrl) } catch (error) {}
                  continue
                }
                currentTotal += image.bytes
                kept.push(image)
              }
            ...
            if (!title && baseImages.length === 0 && added.length === 1) setTitle(added[0].name.replace(/\.[^.]+$/, '').slice(0, 200))
          } catch (error) {
            for (const image of added) {
              uploadPreviewUrlsRef.current.delete(image.previewUrl)
              try { URL.revokeObjectURL(image.previewUrl) } catch (e) {}
            }
            setError({ message: error && error.message ? error.message : '读取图片失败' })
          } finally {
        ...
        const submit = async (overrideText, overrideTitle, overrideDocumentId) => {
          ...
          const payload = {
             ...
             ...(effectiveModelArg ? { model: effectiveModelArg } : {}),
             ...(ontologyArg ? { ontology: ontologyArg } : {}),
            }
          resumeAttemptRef.current = false
        ...
        const appendSubmit = async (overrideText) => {
          ...
          setHistoryOpen(false)
          try {
            const res = await rpc('append-extract', payload)
            if (res && res.error) {
              setPhase('idle')
              setError(res.error)
              return
        ...
        const retryRelations = async () => {
          if (submissionBusyRef.current || taskId || phase === 'extracting') return
          ...
          setActivePara(-1)
          try {
            const res = await rpc('relation-retry', payload)
            if (res && res.error) {
              setPhase('done')
              setError(res.error)
              return
            ...
            setTaskId(res.taskId)
            rememberPendingTask(res.taskId, submittedRef.current)
          } catch (error) {
            setPhase('done')
        ...
        const resetAll = () => {
          setMarkdownBundle(null); setOpenFigure(null)
          ...
          if (bulkActiveTaskRef.current) rpc('task-cancel', { taskId: bulkActiveTaskRef.current }).catch(() => {})
          setBulkReview(null)
        ...
        const persistGraph = (g, baseGraph, pinnedRevision) => {
          ...
          const patch = graphCommitViewPatch(g, baseline)
          const graphPayload = {
          ...
          const describeCommitError = (error) => {
            ...
            const edges = details && Array.isArray(details.edges) ? details.edges : []
            const suffix = edges.length > 0
              ? '：冲突关系 ' + edges.slice(0, 5).join('、')
              : issues.length > 0
          ...
          const queued = graphCommitQueueRef.current.catch(() => {}).then(async () => {
            ...
            const response = await rpc('graph-commit', {
              documentId,
              expectedRevision,
              graph: graphPayload,
              operations: pendingOperations,
              baseNodeIds: patch.baseNodeIds,
              baseEdgeKeys: patch.baseEdgeKeys,
            })
            ...
            if (baseline.view?.kind === 'all') setResultView(current => current?.graph?.nodes === g.nodes
              ...
              ? makeView(asAllNodesGraph({ ...g, revision: response.revision,
                  source: { ...(g.source || {}), revision: response.revision } }), current.sourceText)
              : current)
            rememberLocalGraph()
            return response
          }).catch((error) => {
        ...
        const handleRemoveParagraphType = async (paragraph, type) => {
          ...
          if (!window.confirm('移除 P' + (paragraph + 1) + ' 的“' + label + '”标签及该段对应的 ' + nodes.length + ' 个节点？\n\n' + preview
          ...
          try {
            setError(null)
            const active = await rpc('task-active', {})
            ...
            let graph = meta?.kind === 'all'
              ? asAllNodesGraph({ ...next, revision: response.revision,
                  source: { ...(next.source || {}), revision: response.revision } })
              : response.graph
            let refreshFailed = false
            if (meta) {
              try {
                if (meta.kind === 'all') {
                  const exported = await rpc('document-export', { documentId })
                  if (exported?.error || exported?.revision !== response.revision || !Array.isArray(exported?.graph?.nodes)) {
                    throw new Error('complete graph reload failed')
                  }
                  graph = asAllNodesGraph({ ...exported.graph,
                    graphOntology: baseline.graph.graphOntology || exported.graph.graphOntology })
                } else {
            ...
            if (refreshFailed) setError({ message: meta?.kind === 'all'
              ? '删除已保存，但完整知识图刷新失败；当前显示本次修改后的视图，请重新载入核对。'
              : '删除已保存，但原窗口刷新失败，现显示服务器返回的知识图窗口。' })
          } catch (error) {
        ...
        const commitGraph = (g) => {
          ...
          setResultView(makeView(baseline.view?.kind === 'all' ? asAllNodesGraph(g) : g, src))
          return persistGraph(g, baseline)
        }
        ...
        const startQuickVerify = async () => {
          ...
          try {
            ...
            const res = await rpc('verify-graph', payload)
            if (!isCurrent()) return
        ...
        const startDeepVerify = async () => {
          ...
          try {
            ...
            if (canonicalFull && totalNodes > MAX_VERIFY_NODES) {
              const preview = await rpc('verification-plan', { documentId: selectedDocumentId, expectedRevision: revision })
              if (myGen !== verifyGenRef.current) return
        ...
        const startFactCheck = async () => {
          ...
          try {
            ...
            const res = await rpc('fact-check', payload)
        ...
        const handleCancelExtract = async () => {
          if (!taskId) return
          try {
            const res = await rpc('task-cancel', { taskId })
        ...
        const handleCancelVerify = async () => {
          ...
          if (!id) return
          try {
            const res = await rpc('task-cancel', { taskId: id })
        ...
        const handleCancelFact = async () => {
          if (!factTaskId) return
          try {
            const res = await rpc('task-cancel', { taskId: factTaskId })
        ...
        const submitQuestion = async (draftOverride, targetOverride, reviewIssue = null) => {
          ...
          try {
            ...
            if ((questionGraph.view?.truncated === true && hasNodeReference) || (reviewIssue && documentId)) {
              ...
              const loaded = await rpc('document-export', { documentId, includeSourceText: true })
              if (!isCurrent()) return
            ...
            const res = await rpc('question-graph', payload)
            if (!isCurrent()) return
            if (res && res.error) {
              setQuestionPhase('idle')
        ...
        const runBulkReview = async (initial) => {
          ...
          try {
            const loaded = await rpc('document-export', { documentId: state.documentId, includeSourceText: true })
            ...
            for (let index = state.rows.length; index < state.issueIds.length; index++) {
              ...
              if (!error) {
                ...
                for (;;) {
                  if (bulkStopRef.current) break
                  if (!taskId) {
                    const started = await rpc('question-graph', payload)
                  ...
                  const status = await rpc('task-status', { taskId })
        ...
        const handleApplyBulkReview = async () => {
          ...
          try {
            await graphCommitQueueRef.current.catch(() => {})
            const loaded = await rpc('document-export', { documentId: session.documentId, includeSourceText: true })
            ...
            const refreshedResult = windowed
              ...
              : await rpc('document-export', { documentId: session.documentId, includeSourceText: true })
        ...
        const saveCanonicalIssueReview = async (issue, snapshot, rejectionNote = null) => {
          ...
          try {
            ...
            const refreshed = windowed
              ...
              : await rpc('document-export', { documentId, includeSourceText: true })
            if (!isCurrent()) return
        ...
        const handleApplyAll = async () => {
          ...
          const fixable = open.filter(bulkFixEligible)
          if (fixable.length === 0) {
            toastStore.show('没有可安全批量修复的问题；疑似重复节点需逐条复核')
            return
          }
          if (!window.confirm('将一键应用 ' + fixable.length + ' 个确定性安全修复' + (open.length > fixable.length ? '，另有 ' + (open.length - fixable.length) + ' 个需要人工复核' : '') + '。继续吗？')) return
          bulkFixBusyRef.current = true
          try {
            const res = applyAllFixable(resultView.graph, verificationRef.current || verification)
            if (res.applied === 0) { toastStore.show('没有可应用的安全修复，请重新体检'); return }
            const nextReport = { ...res.report, stale: true }
            const g2 = withVerification(res.graph, nextReport, true)
            setVerification(nextReport)
            setActiveIssueId(null)
            setSelectedNodeId(null)
            setSelectedEdgeId(null)
            if (await commitGraph(g2)) toastStore.show('一键修复已保存：应用 ' + res.applied + ' 项，其余请逐条复核')
          } finally {
        ...
        // Locating a source paragraph (graph node click / issue click / fact
        // claim click) must never yank the whole window: all panels (原文⇄知识图,
        // 章节与候选审核, 验证与质疑, 外部事实核查) are stacked inside the same
        // .kg-win-body scroll container, so centering the paragraph in the body
        // previously scrolled the lower panels into view on a node click.
        // The paragraph is now centred inside the source column only, and the
        // window body is nudged by the minimum amount (nearest edge) just when
        // the paragraph is fully off-screen.
        const scrollElIntoCenter = (el) => {
          let did = false
          const sourceCol = el.closest('.kg-original')
          let colDelta = 0
          if (sourceCol && sourceCol.scrollHeight > sourceCol.clientHeight + 2) {
            const cRect = sourceCol.getBoundingClientRect()
            const tRect = el.getBoundingClientRect()
            colDelta = tRect.top - cRect.top - (cRect.height - tRect.height) / 2
            if (Math.abs(colDelta) > 4) {
              sourceCol.scrollBy({ top: colDelta, behavior: 'smooth' })
              did = true
            }
          }
          const body = el.closest('.kg-win-body')
          if (body && body.scrollHeight > body.clientHeight + 2) {
            const bRect = body.getBoundingClientRect()
            const tRect = el.getBoundingClientRect()
            // Where the paragraph will sit once the source column has scrolled
            // (both deltas derive from the same pre-scroll geometry, so the
            // final position stays predictable while both animations run).
            const finalTop = tRect.top - colDelta
            const finalBottom = finalTop + tRect.height
            let delta = 0
            if (finalBottom < bRect.top) delta = finalTop - bRect.top
            else if (finalTop > bRect.bottom) delta = finalBottom - bRect.bottom
            if (Math.abs(delta) > 4) {
        ...
        const handleSelectNode = (nodeId, projectedAnchor) => {
          ...
          const el = document.getElementById('kg-para-' + pi)
          if (!el) {
            // A cross-chapter neighbor may reveal a paragraph outside the
            // chapter filter on the next render without changing the graph.
            if (projectedAnchor !== undefined) ctx.timeout(() => {
              const target = document.getElementById('kg-para-' + pi)
              if (target) scrollElIntoCenter(target)
            }, 0)
            else toastStore.show('未找到对应原文段落')
            return
          }
        ...
        const locateConsumptionReference = async (reference) => {
          ...
          if (nodeId && !(targetGraph.nodes || []).some((node) => node.id === nodeId)) {
            ...
            await graphCommitQueueRef.current.catch(() => {})
            const loaded = await loadGraphDocument({ documentId, query: nodeId, nodeOffset: 0, includeSourceText: false })
            if (!loaded || loaded.error || !loaded.graph || !Array.isArray(loaded.graph.nodes)) throw new Error(loaded && loaded.error && loaded.error.message ? loaded.error.message : '无法加载节点所在子图')
            if (!loaded.graph.nodes.some((node) => node.id === nodeId)) throw new Error('canonical knowledge graph 中找不到节点 ' + nodeId)
            const source = fullText || resultView.sourceText || ''
            targetGraph = loaded.graph
            targetView = makeView(targetGraph, source)
            graphRevisionRef.current = Number.isInteger(loaded.revision)
              ? loaded.revision
              : (targetGraph.source && Number.isInteger(targetGraph.source.revision) ? targetGraph.source.revision : graphRevisionRef.current)
            setResultView(targetView)
            setGraphQueryDraft(nodeId)
            const meta = graphViewMetadata(targetGraph)
            if (meta) setGraphPageDraft(String(meta.page))
          }
          ...
          if (Number.isInteger(paragraph) && paragraph >= 0) {
            setActivePara(paragraph)
            setFlashPara(paragraph)
            ctx.timeout(() => {
              const el = document.getElementById('kg-para-' + paragraph)
              if (el) scrollElIntoCenter(el)
            }, 60)
        ...
        const handleCandidateReview = async (key, status) => {
          ...
          const source = resultView.graph.source && typeof resultView.graph.source === 'object' ? resultView.graph.source : {}
          try {
            const res = await rpc('candidate-update', {
        ...
        const loadHistoryEntry = async (entry) => {
          ...
          setError(null)
          try {
            const loaded = await loadGraphDocument({ documentId: entry.documentId })
            if (!loaded || loaded.error || !loaded.graph || !Array.isArray(loaded.graph.nodes)) {
        ...
        const inputPanel = h('section', { className: 'kg-card', 'aria-label': '输入资料' },
          ...
          inputCollapsed
            ? h('p', { className: 'kg-hint', style: { margin: 0 } },
                '输入区已收起' + (title ? '（标题：' + title + '）' : '') + ' · 正文 ' + text.length + ' 字' + (imageInputs.length > 0 ? ' · 图片 ' + imageInputs.length + ' 张' : ''))
            : h(React.Fragment, null,
                h('input', {
                ...
                h('div', { className: 'kg-actions' },
                  ...
                  !resultView && imageInputs.length > 0 ? h('div', { className: 'kg-image-list', 'aria-label': '已选择图片' },
                     imageInputs.map((image) => h('div', { key: image.id, className: 'kg-image-item' },
                       h('img', { className: 'kg-image-thumb', src: image.previewUrl, alt: image.name }),
                       h('div', { className: 'kg-image-meta' },
                         h('span', { className: 'kg-image-name', title: image.name }, image.name),
                         h('span', { className: 'kg-image-size' }, imageBytesLabel(image.bytes) + ' · ' + image.mediaType.replace('image/', '').toUpperCase()),
                       ),
                       h('button', { type: 'button', className: 'kg-image-remove', title: '移除图片', 'aria-label': '移除图片 ' + image.name, onClick: () => removeImageInput(image.id) }, '×'),
                     )),
                   ) : null,
                   !resultView && imageInputs.length > 0 ? h('p', { className: 'kg-visual-note' }, '图片会先由多模态模型忠实转写可见文字、表格结构和图中连线，再进入知识图抽取。视觉转写可能有识别误差，请结合原图核对。') : null,
                   !resultView ? h(React.Fragment, null,
                     h('input', { ref: folderInputRef, type: 'file', multiple: true, webkitdirectory: '', className: 'kg-image-input', onChange: importMarkdownFolder, 'aria-label': '选择 Markdown 与图片目录' }),
                     h('button', { type: 'button', className: 'kg-secondary', disabled: folderPreparing || pdfPreparing || imagePreparing, onClick: () => folderInputRef.current?.click() }, folderPreparing ? '正在导入图文…' : '导入图文目录'),
                     h('input', { ref: pdfInputRef, className: 'kg-image-input', type: 'file', accept: PDF_ACCEPT, onChange: importPdfFile, 'aria-label': '上传 PDF 生成知识图' }),
                     h('button', { type: 'button', className: 'kg-secondary kg-image-upload-btn', disabled: pdfPreparing || imagePreparing, onClick: () => pdfInputRef.current && pdfInputRef.current.click() }, pdfPreparing ? '⏳ 正在解析 PDF…' : '📄 上传 PDF'),
                     h('input', { ref: imageInputRef, className: 'kg-image-input', type: 'file', accept: IMAGE_ACCEPT, multiple: true, onChange: addImageFiles, 'aria-label': '上传图片生成知识图' }),
                     h('button', { type: 'button', className: 'kg-secondary kg-image-upload-btn', disabled: pdfPreparing || imagePreparing || imageInputs.length >= MAX_IMAGE_INPUTS, onClick: () => imageInputRef.current && imageInputRef.current.click() }, imagePreparing ? '⏳ 正在读取…' : '🖼 上传图片'),
                   ) : null,
                  ...
                  resultView
                    ...
                    : h('button', {
                        ...
                        disabled: resultView ? text.trim().length === 0 : (text.trim().length === 0 && imageInputs.length === 0),
                        onClick: () => submit(),
                      }, imageInputs.length > 0 ? '图片生成知识图' : 'AI 拆分'),
                ),
              ),
        )
              ...
              return h('section', { className: 'kg-card kg-result', 'aria-label': '原文 ⇄ 知识图结果' },
                ...
                h('div', { className: 'kg-stats' },
                  ...
                  h(GraphExportActions, { graph: resultView.graph, title, ctx,
                    loadCanonical: (documentId) => rpc('document-export', { documentId }) }),
                  diagCount > 0
                    ? h('button', {
                ...
                windowMeta
                  ? h('div', { className: 'kg-window-nav', 'aria-label': '大图窗口导航与子图查询' },
                      ...
                      allNodesActive
                        ? h('strong', null, '全部节点 · ' + displayView.graph.nodes.length + ' 个')
                ...
                h('div', {
                  ...
                  h('div', { className: 'kg-original', 'aria-label': '原文段落', style: { maxHeight: resultHeight + 'px' } },
                    visualImages.length > 0 && visualMeta.kind !== 'markdown-assets' ? h(React.Fragment, null,
                       h('p', { className: 'kg-visual-note', style: { margin: 8 } }, '以下原文是多模态模型从图片生成的视觉转写，可能有识别误差。点击图片可定位其转写段落，请对照原图复核关键证据。'),
                       h('div', { className: 'kg-source-gallery', 'aria-label': '原始图片' }, visualImages.map((image) => {
                         const start = Number.isInteger(image.startParagraph) ? image.startParagraph : -1
                         const end = Number.isInteger(image.endParagraph) ? image.endParagraph : start
                         return h('button', {
                           key: image.id, type: 'button',
                           className: 'kg-source-image' + (activePara >= start && activePara <= end ? ' on' : ''),
                           title: '定位图片对应的视觉转写段落',
                           onClick: () => {
                             if (start < 0) return
                             setActivePara(start); setFlashPara(start)
                             const el = document.getElementById('kg-para-' + start)
                             if (el) scrollElIntoCenter(el)
                             ctx.timeout(() => setFlashPara(-1), 1400)
                           },
                         },
                           sourceImageUrls[image.id] ? h('img', { src: sourceImageUrls[image.id], alt: image.name || image.id }) : h('span', { style: { height: 92, display: 'grid', placeItems: 'center' } }, '图片加载中…'),
                           h('span', { title: image.name || image.id }, image.name || image.id),
                         )
                       })),
                     ) : null,
                  ...
                  h('div', { className: 'kg-graph-col' },
                    ...
                    h(GraphViewer, {
                      ...
                      loadNeighborhood: async (args, signal) => {
                        ...
                        return rpc('graph-neighborhood', args, { signal })
                      },
                      onGatherProjection: setGatherProjection,
                      onGatherEnd: () => { setActivePara(activePara); setFlashPara(-1); setSelectedNodeId(selectedNodeId); setSelectedEdgeId(selectedEdgeId); setFocusReq(focusReq) },
        ...
        return h(React.Fragment, null,
          h('div', { className: 'kg-body-toolbar' },
            h('div', { className: 'kg-body-toolbar-text' },
              ...
              h('p', { className: 'kg-subtitle' },
                shownOntology && !shownOntology.isDefault
                  ? shownOntology.label + '：' + (shownOntology.summary || '') + '（' + shownOntology.nodeTypes + ' 类节点 · ' + shownOntology.relationTypes + ' 类关系）'
            ...
            h('div', { style: { display: 'flex', gap: 8, flex: 'none', maxWidth: '100%', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' } },
              ...
              // Which ontology a NEW extraction is generated with. Only rendered
              // when the host offers more than one, so a single-ontology install
              // sees no extra control.
              ontologyOptions.length > 1
                ? h('label', { style: { display: 'flex', alignItems: 'center', gap: 6 } }, '知识图模式',
                    h('select', {
                      value: ontologyChoice || defaultOntologyId,
                      disabled: phase === 'extracting',
                      onChange: (e) => handleOntologyChange(e.target.value),
                      'aria-label': '知识图模式',
                      title: '只影响新抽取；已生成的知识图按它自己被抽取时的本体显示',
              ...
              h('button', { type: 'button', className: 'kg-secondary', disabled: !!taskId || phase === 'extracting' || phase === 'paused', onClick: () => setHistoryOpen(!historyOpen) },
                historyOpen ? '返回工作台' : '历史'),
              resultView ? h('button', { type: 'button', className: 'kg-secondary', disabled: !!taskId || phase === 'extracting' || phase === 'paused', onClick: resetAll }, '重新开始') : null,
            ),
          ),
          ...
          openFigure && resultView ? h('dialog', {
            ...
            'aria-label': '原始插图', style: { width: 'min(960px, 90vw)', maxHeight: '90vh', padding: 16, border: '1px solid #aaa', borderRadius: 8 },
          },
                ...
                relationTaskViewing
                  ? h('section', { className: 'kg-relation-progress', 'aria-label': '关系补全进度' },
                      ...
                      h('details', null,
                        ...
                        extractProgress?.warning ? h('p', { className: 'kg-empty-sub' }, extractProgress.warning) : null,
      ...
      function TrajectoryTab({ sessionId }) {
        ...
        const applyHydratedTrajectory = (graph, sourceText, events, revision) => {
          ...
          const evs = normalizeTraceEvents(Array.isArray(rawEvents) ? rawEvents : [])
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            ...
            let res = null
            try {
              res = await rpc('trajectory-status', { taskId })
            } catch (e) {
            ...
            if (res && res.status === 'succeeded') {
              ...
              if (g && Array.isArray(g.nodes)) {
                ...
                const evs = normalizeTraceEvents(Array.isArray(g.traceEvents) ? g.traceEvents : [])
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            ...
            let res = null
            try { res = await rpc('task-status', { taskId: questionTaskId }) }
            catch (e) {
              if (disposed || mySeq !== sessionSeq.current || myGen !== verifyGenRef.current) return
              setQuestionPhase('idle'); setQuestionTaskId(null)
              setQuestionError('查询质疑任务失败：' + (e && e.message ? e.message : '未知错误'))
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            ...
            let res = null
            try { res = await rpc('task-status', { taskId: factTaskId }) }
            catch (e) {
              if (disposed || mySeq !== sessionSeq.current || myGen !== factGenRef.current) return
              setFactPhase('idle'); setFactTaskId(null)
              setError({ message: '查询外部核查任务失败：' + (e && e.message ? e.message : '未知错误') })
        ...
        const extract = async () => {
          ...
          clearTrajResult(sessionId)
          try {
            const res = await rpc('trajectory-extract', { sessionId, ...(effectiveModelArg ? { model: effectiveModelArg } : {}) })
        ...
        const appendExtract = async () => {
          ...
          try {
            ...
            const res = await rpc('trajectory-append-extract', {
              sessionId,
              documentId,
        ...
        const startQuickVerify = async () => {
          ...
          try {
            ...
            const res = await rpc('verify-graph', {
        ...
        const startFactCheck = async () => {
          ...
          setFactPhase('running')
          try {
            const res = await rpc('fact-check', {
              title: '', text: view.sourceText || '',
              graph: { ontology: view.graph.ontology || view.graph.source?.ontology || view.graph.graphMeta?.ontology,
                summary: view.graph.summary || '', nodes: view.graph.nodes, edges: view.graph.edges },
              mode: 'deep',
        ...
        const locateTrajectoryConsumptionReference = async (reference) => {
          ...
          const nodeId = typeof reference.nodeId === 'string' ? reference.nodeId : ''
          let targetView = view
          if (nodeId && !(view.graph.nodes || []).some((node) => node.id === nodeId)) {
            const documentId = documentIdOfGraph(view.graph)
            if (!documentId) throw new Error('该节点不在当前视图，且轨迹图没有 canonical documentId')
            await trajCommitQueueRef.current.catch(() => {})
            const loaded = await loadGraphDocument({ documentId, query: nodeId, nodeOffset: 0, includeSourceText: false })
            if (!loaded || loaded.error || !loaded.graph || !Array.isArray(loaded.graph.nodes)) throw new Error(loaded && loaded.error && loaded.error.message ? loaded.error.message : '无法加载节点所在子图')
            if (!loaded.graph.nodes.some((node) => node.id === nodeId)) throw new Error('canonical trajectory graph 中找不到节点 ' + nodeId)
            targetView = makeView(loaded.graph, view.sourceText || '')
            trajRevisionRef.current = Number.isInteger(loaded.revision)
              ? loaded.revision
              : (loaded.graph.source && Number.isInteger(loaded.graph.source.revision) ? loaded.graph.source.revision : trajRevisionRef.current)
            setView(targetView)
          }
          if (nodeId) {
            setSelectedNodeId(nodeId)
          ...
          if (Number.isInteger(paragraph) && paragraph >= 0) {
            ...
            setFlashPara(paragraph)
            ctx.timeout(() => {
              const el = document.getElementById('kg-traj-para-' + paragraph)
              if (el) scrollElIntoCenter(el)
            }, 60)
            ctx.timeout(() => setFlashPara(-1), 1460)
          }
                ...
                h('div', {
                  ...
                  h('div', { className: 'kg-graph-col' },
                    h(GraphViewer, {
                      ...
                      documentId: documentIdOfGraph(view.graph), revision: view.graph.revision ?? view.graph.source?.revision,
          ...
          phase === 'extracting' || phase === 'paused'
            ...
            : view
              ? h('section', { className: 'kg-card kg-result', 'aria-label': '轨迹 ⇄ 知识图结果' },
                  ...
                  resultPanel,
                  h(KnowledgeConsumePanel, {
                    ctx, graph: view.graph, sourceText: view.sourceText || '',
                    mode: 'trajectory', model: effectiveModelArg,
