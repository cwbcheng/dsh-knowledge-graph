      ...
      function WorkbenchBody({ ctx }) {
        ...
        useEffect(() => {
          ...
          ;(async () => {
            ...
            if (saved && typeof saved.documentId === 'string' && saved.documentId) {
              try {
                const loaded = await rpc('document-load', { documentId: saved.documentId })
                if (disposed) return
                if (loaded && !loaded.error && loaded.graph && Array.isArray(loaded.graph.nodes)) {
        ...
        useEffect(() => {
         ...
         const loadGraphWindow = async ({ page, query, nodeLimit } = {}) => {
           ...
           try {
             ...
             const loaded = await rpc('document-load', {
               documentId,
               nodeOffset,
               nodeLimit: limit,
          ...
          const tick = async () => {
            ...
            if (res && res.status === 'succeeded') {
              ...
              if (g && Array.isArray(g.nodes)) {
                ...
                if (documentId) {
                  try {
                    const loaded = await rpc('document-load', { documentId })
                    if (disposed) return
                    if (loaded && !loaded.error && loaded.graph && Array.isArray(loaded.graph.nodes)) {
                      g = loaded.graph
        ...
        const locateConsumptionReference = async (reference) => {
          ...
          if (nodeId && !(targetGraph.nodes || []).some((node) => node.id === nodeId)) {
            ...
            await graphCommitQueueRef.current.catch(() => {})
            const loaded = await rpc('document-load', { documentId, query: nodeId, nodeOffset: 0, includeSourceText: false })
        ...
        const loadHistoryEntry = async (entry) => {
          ...
          setError(null)
          try {
            const loaded = await rpc('document-load', { documentId: entry.documentId })
            if (!loaded || loaded.error || !loaded.graph || !Array.isArray(loaded.graph.nodes)) {
