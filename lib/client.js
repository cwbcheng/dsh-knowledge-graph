...
`)
      ...
      function layoutForce(nodes, edges, sizes) {
        ...
        // Post-pass A — edge-node repulsion: keep every node off the lines of
        // edges it does not touch, so arrows stop slicing through nodes.
        if (n > 1) {
          ...
          for (let iter = 0; iter < 60; iter++) {
            ...
            for (const id of ids) {
              const p = pos.get(id)
              const moveX = clamp(fx.get(id), -14, 14)
              const moveY = clamp(fy.get(id), -14, 14)
              if (Math.abs(moveX) > 0.05 || Math.abs(moveY) > 0.05) moved += 1
              p.x += moveX
              p.y += moveY
            }
            if (moved === 0) break
      ...
      function layeredOrthoPath(edge, a, b, sizes, pos, nodes, lane) {
        ...
        if (r1 === r2) {
          ...
          const cye = cy + offY
          const d = 'M ' + (a.x + offX) + ' ' + a.y + ' L ' + (a.x + offX) + ' ' + cye
            + ' L ' + (b.x + offX) + ' ' + cye + ' L ' + (b.x + offX) + ' ' + b.y
        ...
        const ch1e = ch1 + offY
        const ch2e = ch2 + offY
        const d = 'M ' + (a.x + offX) + ' ' + a.y + ' L ' + (a.x + offX) + ' ' + ch1e
          + ' L ' + xce + ' ' + ch1e + ' L ' + xce + ' ' + ch2e
          + ' L ' + (b.x + offX) + ' ' + ch2e + ' L ' + (b.x + offX) + ' ' + b.y
        return { d, lblX: xce + 14, lblY: (ch1e + ch2e) / 2 }
      }

      function layoutGraph(nodes, edges, sizes, mode) {
        if (mode === 'circular') return { pos: resolveNodeOverlaps(nodes, sizes, layoutCircular(nodes, edges, sizes), 18) }
        if (mode === 'radial') return { pos: resolveNodeOverlaps(nodes, sizes, layoutRadial(nodes, edges, sizes), 18) }
        if (mode === 'layered') return { pos: resolveNodeOverlaps(nodes, sizes, layoutLayered(nodes, edges, sizes), 18) }
      ...
      function GraphViewer({ nodes, edges, anchors, selectedNodeId, selectedEdgeId, focusReq, onSelectNode, onSelectEdge, ctx, height, layoutMode, onLayoutModeChange }) {
        ...
        // Per-edge lanes for the orthogonal layered routing: edges sharing a
        // row pair get a lane index so their channel/corridor runs separate by
        // a few px instead of stacking into one indistinguishable line.
        const layeredLanes = useMemo(() => {
          const lanes = new Map()
          if (layoutMode !== 'layered') return lanes
          const groups = new Map()
          const rowOf = (p) => Math.round(p.y / LAYER_Y_GAP)
          for (const edge of edges || []) {
            const a = layout.pos.get(edge.fromNodeId)
            ...
            const r1 = rowOf(a)
            const r2 = rowOf(b)
            const key = r1 === r2 ? 'row' + r1 : Math.min(r1, r2) + '>' + Math.max(r1, r2)
        ...
        const edgeEls = (edges || []).map((edge, i) => {
          ...
          if (layoutMode === 'layered') {
            const o = layeredOrthoPath(edge, a, b, sizes, layout.pos, nodes, layeredLanes.get(edge) || 0)
            d = o.d
            lblX = o.lblX
            lblY = o.lblY
