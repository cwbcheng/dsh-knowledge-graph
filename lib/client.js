      ...
      function WorkbenchBody({ ctx }) {
        ...
        useEffect(() => {
         ...
         const resumeLostTask = async (statusResult) => {
           ...
           try {
             const resumed = await rpc('extract', payload)
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            ...
            let res = null
            try {
              res = await rpc('task-status', { taskId, includeCheckpoint: true })
            } catch (e) {
              if (disposed) return
              setPhase('idle')
              setTaskId(null)
        ...
        const verificationSourcePayload = (source, graph) => {
          if (typeof source !== 'string' || source.length <= MAX_LEN) return {}
          ...
          return sourceUnits.length > 0 ? { sourceUnits } : {}
        }
