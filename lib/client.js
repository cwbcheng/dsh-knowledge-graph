...
`)
      ...
      function WorkbenchBody({ ctx }) {
        ...
        const [allNodesGraph, setAllNodesGraph] = useState(null)
        ...
        useEffect(() => {
         ...
         const locateAllNode = async id => {
           ...
           const meta = graphViewMetadata(resultView?.graph)
           if (index < 0 || !meta) return
           const page = Math.floor(index / meta.nodeLimit) + 1
           const loaded = await loadGraphWindow({ page, nodeLimit: meta.nodeLimit, query: '' })
           if (loaded) {
             setSelectedNodeId(id)
             setSelectedEdgeId(null)
             setFocusReq(value => ({ nodeId: id, seq: value.seq + 1 }))
