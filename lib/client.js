...
.kg-issue { border: 1px solid var(--kg-border); border-left: 4px solid #94a3b8; border-radius: 10px; padding: 9px 12px; margin-bottom: 8px; background: var(--kg-panel); cursor: pointer; }
.kg-issue:hover, .kg-issue.on { border-color: rgba(59,130,246,0.6); }
.kg-issue.kg-sev-error { border-left-color: #dc2626; }
.kg-issue.kg-sev-warning { border-left-color: #d97706; }
.kg-issue.kg-sev-suggestion { border-left-color: #2563eb; }
.kg-issue-top { display: flex; align-items: center; flex-wrap: wrap; gap: 6px; }
.kg-sev-tag { display: inline-flex; padding: 0 8px; border-radius: 999px; color: #fff; font-size: 11px; line-height: 18px; font-weight: 600; }
.kg-sev-error { background: #dc2626; } .kg-sev-warning { background: #d97706; } .kg-sev-suggestion { background: #2563eb; }
.kg-issue-cat { font-size: 11px; color: var(--kg-text-dim); border: 1px solid var(--kg-border); border-radius: 999px; padding: 0 8px; line-height: 18px; }
...
.kg-verdict { display: inline-flex; padding: 0 8px; border-radius: 999px; color: #fff; font-size: 11px; line-height: 18px; font-weight: 600; }
.kg-verdict-supported { background: #059669; } .kg-verdict-contradicted { background: #dc2626; } .kg-verdict-insufficient { background: #d97706; } .kg-verdict-out_of_scope { background: #64748b; }
...
`)
      ...
      function TrajectoryTab({ sessionId }) {
        ...
        const extract = async () => {
          ...
          clearTrajResult(sessionId)
          try {
            const res = await rpc('trajectory-extract', { sessionId })
        ...
        const appendExtract = async () => {
          ...
          try {
            ...
            const res = await rpc('trajectory-append-extract', { sessionId, existing })
            if (res && res.error) { setPhase('idle'); setError(res.error); return }
            if (res && res.taskId) {
              setTaskId(res.taskId)
        ...
        const handleApplyIssue = (issue) => {
          ...
          if (next === view.graph) { showToast('该问题没有可应用的补丁'); return }
          let report = verification
