...
`)
      ...
      function SourceFigure({ image, documentId, revision, onOpen, large }) {
        ...
        useEffect(() => {
          ...
          setError(''); setUrl('')
          rpc('image-load', { documentId, imageId: image.id, expectedRevision: revision }).then(response => {
            if (disposed) return
        ...
        useEffect(() => {
          ...
          ;(async () => {
            ...
            for (const image of images) {
              ...
              try {
                const response = await rpc('image-load', { documentId, imageId: image.id, ...(Number.isInteger(revision) ? { expectedRevision: revision } : {}) })
