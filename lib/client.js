...
`)
      ...
      function WorkbenchBody({ ctx }) {
        ...
        useEffect(() => {
          ...
          ;(async () => {
            ...
            if (saved && typeof saved.documentId === 'string' && saved.documentId) {
              try {
                const loaded = await loadGraphDocument({ documentId: saved.documentId })
                if (disposed) return
                if (loaded && !loaded.error && loaded.graph && Array.isArray(loaded.graph.nodes)) {
        ...
        useEffect(() => {
         ...
         const loadGraphWindow = async ({ page, query, nodeLimit } = {}) => {
         ...
         const openAllNodes = async () => {
           if (!resultView || resultView.graph.view?.kind === 'all' || allNodesLoading || graphWindowLoading) return
           ...
           try {
             ...
             if (currentResultRef.current !== openingView) throw new Error('知识图视图已变化，请在当前视图重新选择全部节点')
             if (response.documentId !== documentId || response.revision !== expectedRevision || graphRevisionRef.current !== expectedRevision) {
                ...
                h('div', {
                  ...
                  h('div', { className: 'kg-graph-col' },
                    ...
                    h(GraphViewer, {
                      ...
                      onQuestionNode: handleQuestionNode,
                      onQuestionEdge: handleQuestionEdge,
