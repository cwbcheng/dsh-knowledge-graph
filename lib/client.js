          ...
          const tick = async () => {
            ...
            if (res && res.status === 'succeeded') {
              ...
              if (g && Array.isArray(g.nodes)) {
                ...
                setExtractProgress(null)
                setInputCollapsed(true)
                const entryId = sub.append === true && currentHistoryId ? currentHistoryId : 'h-' + Date.now()
      ...
      function TrajectoryTab({ sessionId }) {
        ...
        const appendExtract = async () => {
          ...
          try {
            ...
            const res = await rpc('trajectory-append-extract', { sessionId, existing, ...(effectiveModelArg ? { model: effectiveModelArg } : {}) })
            if (res && res.error) { setPhase('idle'); setError(res.error); return }
            if (res && res.taskId) {
              setTaskId(res.taskId)
