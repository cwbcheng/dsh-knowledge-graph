...
export function apply(ctx) {
      ...
      // ---- content-aware paragraph segmentation ----
      // A "paragraph" for the AI prompt is no longer only a blank-line block:
      // long natural paragraphs are split at sentence boundaries and, for
      // pathological run-on sentences, at soft punctuation, so each numbered
      // unit stays small enough to anchor nodes precisely. The client MUST
      // mirror this exact algorithm (src/index.client.js splitParagraphs).
      const SEG_TARGET_HOST = 180
      const SEG_MAX_HOST = 300
      const SENT_END_HOST = new Set(['。', '！', '？', '!', '?', '；', ';'])
      const SENT_CLOSER_HOST = new Set(['”', '’', '"', "'", '」', '』', '）', ')', '】', '》', '〉'])
      const SEG_SOFT_HOST = new Set(['，', '、', '：', ':', ',', '—', '…', ' ', '\t'])

      // Split one atomic sentence (already shorter than what the caller keeps)
      // if it still exceeds SEG_MAX_HOST; prefer soft punctuation, hard-cut as
      // a last resort.
      ...
      function splitParagraphsOffsetsHost(text) {
        ...
        const flush = () => {
          ...
          for (const p of parts) {
            ...
            if (mergedLen <= SEG_TARGET_HOST) {
              curEnd = p.end
            } else {
              const piece = text.slice(curStart, curEnd)
        ...
        for (const line of lines) {
          ...
          for (const s of sentences) {
            splitLongSentenceHost(line.slice(s.start, s.end), lineStart + s.start, parts)
          }
          lineStart += line.length + 1
        }
        flush()
