...
const CONSUME_NODE_TYPES = new Set(['fact', 'claim', 'inference', 'concept', 'definition', 'example', 'counter_example', 'rule'])
const CONSUME_RELATIONS = new Set(['supports', 'example', 'counter_example', 'defines', 'infers', 'causes', 'is_a', 'contains', 'driven_by', 'not_is', 'analogy', 'aims_at'])
...
export class SqliteKnowledgeStore {
  ...
  queryDocumentGraph(documentId, options = {}) {
    ...
    return {
      ...
      sourceText: options.includeSourceText === false ? '' : (row.source_text || ''),
      revision,
      nodes: Array.from(selectedRows.values()).map(nodeFromRow),
      edges: returnedEdges.map(edgeFromRow),
      matches: direct.map((item) => ({ nodeId: item.row.node_id, score: item.score, reasons: item.reasons.slice(0, 4) })),
      view: {
        kind: 'consumption', query: queryRaw, directMatches: direct.length,
        candidateMatches: candidateCount, totalNodes, totalEdges,
        returnedNodes: selectedRows.size, returnedEdges: returnedEdges.length,
        hops, direction,
