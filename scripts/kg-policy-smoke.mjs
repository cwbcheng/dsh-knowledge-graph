...
assert(host.includes('const evaluated = validateGraphInvariantsHost(graph, sourceText, { includeQuality: true, skipPairChecks: true, sourceUnitLengths })') && host.includes('await buildLocalReportBatchedHost(graph, text, undefined, input.sourceUnitLengths)'), 'batched quick-check must share the invariant evaluator and scoped source boundaries')
...
assert(host.includes('buildConsumptionEvidenceCatalogHost') && host.includes('part.evidenceIds') && host.includes('usedEvidenceIds'), 'answer admission is not evidence-ID based')
assert(host.includes('function consumptionEvidenceSupportsPartHost') && host.includes('function consumptionSupportedEvidenceIdsHost') && host.includes("consumptionSupportedEvidenceIdsHost(text, ids, evidenceById, raw.status === 'insufficient')") && host.includes('consumptionRestatesQuotedQuestionHost') && host.includes('directAnswerApproved'), 'answer admission does not verify evidence per clause and reject quoted-question restatements')
assert(host.includes('CONSUMPTION_ANSWER_REVIEW_SYSTEM_PROMPT') && host.includes('normalizeConsumptionAnswerReviewHost') && host.includes("approval.verdict === 'supported'") && host.includes('natural ? candidate.text : consumptionRenderedPartTextHost') && !host.includes("status !== 'answered' && typeof obj.answer") && !host.includes('Array.isArray(obj.followUps)'), 'unreviewed model prose or follow-ups can surface outside authenticated evidence')
