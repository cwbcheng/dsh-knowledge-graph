...
assert(store.includes('getDocumentRevision(documentId)') && buildLib.includes('const baseRevision = requestedDocumentId ? (await getSqliteStore()).getDocumentRevision(requestedDocumentId) : 0') && buildLib.includes('baseRevision: checkpoint.baseRevision'), 'persistent extraction/resume does not fence every canonical replacement by its start revision')
assert(host.includes('const baseDocument = requestedDocumentId ? loadCanonicalDocumentHost(requestedDocumentId) : null') && host.includes("if (Number.isInteger(task.baseRevision) && typeof persistGraph !== 'function')"), 'dynamic replacement tasks do not enforce the same start-revision fence')
