...
try {
  ...
  const saved = store.saveGraph(graph)
  if (saved.nodes !== 2 || saved.entityCandidates !== 1 || saved.claimCandidates !== 1) throw new Error('graph persistence counts are wrong: ' + JSON.stringify(saved))
  ...
  const checkpoint = { version: 1, documentId: 'document-smoke', sourceId: 'source-smoke', nextBatchIndex: 1, totalBatches: 2, graph, staging: graph.staging }
  const run = store.saveCheckpoint(checkpoint, { runId: 'run-smoke' })
  const loaded = store.loadCheckpoint('run-smoke')
  if (!loaded || loaded.checkpoint.nextBatchIndex !== 1) throw new Error('checkpoint persistence failed')
  const restored = store.getDocument('document-smoke')
  if (!restored || restored.nodes.length !== 2 || restored.edges.length !== 1 || restored.staging.chunks.length !== 2) throw new Error('document restore failed')
  console.log(JSON.stringify({ ok: true, saved, candidates: candidates.length, accepted, run, restoredNodes: restored.nodes.length }))
} finally {
  store.close()
}
