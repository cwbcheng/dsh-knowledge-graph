...
`)
      ...
      function BackgroundTaskProgress({ ctx, knownTaskIds = [], busyError, onOpenDocument, onStopTask }) {
        ...
        useEffect(() => {
          ...
          const tick = async () => {
            try {
              const response = await host.call('task-active', { taskId: observedId.current })
              if (disposed) return
      ...
      function KnowledgeConsumePanel({ ctx, graph, sourceText, mode, model, onLocateReference, onNotify }) {
        ...
        const cancelAsk = async () => {
          ...
          askTaskRef.current = null
          if (taskId) {
            try { await host.call('task-cancel', { taskId }) } catch (e) {}
          }
          setAskState({ phase: 'cancelled', taskId: null, progress: null, result: null, error: null })
        }
        const clearSearch = () => {
      ...
      function WorkbenchBody({ ctx }) {
        ...
        useEffect(() => {
         ...
         const openAllNodes = async (refresh = false) => {
           ...
           try {
             ...
             const response = await host.call('document-export', { documentId })
        ...
        const importMarkdownFolder = async event => {
          ...
          try {
            ...
            const response = await host.call('markdown-import', { text: await md.text(), files: uploads })
        ...
        const importPdfFile = async (event) => {
          ...
          try {
            ...
            const res = await host.call('document-import', { uploads: [{ name: prepared.name, mediaType: prepared.mediaType, data: prepared.data }] })
            if (res && res.error) {
        ...
        const handleRemoveParagraphType = async (paragraph, type) => {
          ...
          try {
            ...
            if (meta) {
              try {
                if (meta.kind === 'all') {
                  const exported = await host.call('document-export', { documentId })
