...
function createHostPlugin(graphContractOnly) {
  return {
    ...
    apply(ctx) {
      ...
      // Pairwise review yields between bounded batches; the request cap still
      // limits total work and report size independently of the render window.
      ...
      const RELATION_WEAVE_RATE_LIMIT_DELAY_MS = 30000
       ...
       const MAX_DIRECT_DOCUMENT_UPLOADS_HOST = 4
       const MAX_DIRECT_DOCUMENT_TOTAL_BYTES_HOST = 20 * 1024 * 1024
      ...
      const ONTOLOGY_PROFILES = Object.freeze({
        ...
        "diagnostics": [
          ...
          },
          {
            "id": "mapping_missing",
            "zh": "联结空载",
            "definition": "联结模型没有交代它在什么与什么之间映射，映射只写在文字里。"
          },
          {
            "id": "memorize_words",
            "zh": "记言代学",
      ...
      // <<< END GENERATED ONTOLOGY DATA <<<
       ...
       function graphNeighborhoodHost(documentId, saved, options) {
         ...
         const graph = saved.graph, centerId = options.centerId, direction = options.direction || 'both', relation = options.relation || '', hops = options.hops ?? 1
         ...
         for (const edge of graph.edges || []) {
           ...
           if (direction !== 'in') {
             ...
             adjacency.get(edge.fromNodeId).push({ id: edge.toNodeId, relation: edge.relation })
           }
           if (direction !== 'out') {
             if (!adjacency.has(edge.toNodeId)) adjacency.set(edge.toNodeId, [])
             adjacency.get(edge.toNodeId).push({ id: edge.fromNodeId, relation: edge.relation })
           }
         }
         ...
         for (let depth = 1; depth <= hops && frontier.length; depth++) {
           const next = []
           for (const from of frontier) for (const edge of adjacency.get(from) || []) {
             relationTypesSet.add(edge.relation)
             if (relation && edge.relation !== relation) continue
             if (!depthById.has(edge.id)) { depthById.set(edge.id, depth); next.push(edge.id) }
             if (depthById.get(edge.id) === depth) {
               const previous = neighborRelations.get(edge.id)
               if (previous === undefined || edge.relation < previous) neighborRelations.set(edge.id, edge.relation)
             }
           }
           frontier = next
         }
         const cmp = (a, b) => a < b ? -1 : a > b ? 1 : 0
         const neighbors = [...neighborRelations.keys()].sort((a, b) => depthById.get(a) - depthById.get(b) || cmp(neighborRelations.get(a), neighborRelations.get(b)) ||
         ...
         const offset = Math.min(options.offset ?? 0, visibleTotal), page = neighbors.slice(offset, Math.min(visibleTotal, offset + (options.limit ?? 80)))
         ...
         return { documentId, revision: saved.revision, centerId, direction, relation, hops,
       ...
       function newlyInvalidOntologyEdgesHost(before, after) {
         const rules = new Map((ontProfile(before).relationTypes || []).map((relation) => [relation.id, relation]))
         const valid = (edge, nodes) => {
           const rule = rules.get(edge.relation)
         ...
         const existingViolations = new Set((before.edges || []).filter((edge) => !valid(edge, oldNodes)).map(edgeKey))
         return (after.edges || []).filter((edge) => !valid(edge, nextNodes) && !existingViolations.has(edgeKey(edge)))
           .map(edgeKey)
       }
      ...
      const LEARNING_VIEW_SYSTEM_PROMPT = (() => {
        ...
        return [
          ...
          '6. 判别模型与联结模型分开建：不要用一个含糊的「模型」节点带过。两者混同会让「判联错配」无法诊断。联结模型还必须交代输入与输出（maps_between 或 factor_material），否则会触发「联结空载」。',
      ...
      const MAX_FIX_QUOTE_CHARS = 600
      const FIX_TEXT_LIMIT_PROMPT = '修复字段长度上限（按 JavaScript UTF-16 长度计）：nodePatch.patch.text 和 summaryPatch 不超过 '
        + MAX_FIX_TEXT_CHARS + '，nodePatch.patch.quote 不超过 ' + MAX_FIX_QUOTE_CHARS
        + '。必须保留完整语义，尤其否定、条件和适用范围；quote 必须是完整且有依据的逐字摘录，不得截取前缀冒充完整修复。无法在限额内安全表述时返回 none 并说明原因。超限补丁会被整体拒绝，不会截断或只应用其中部分字段。'
      ...
      const LEARNING_VIEW_QUESTION_SYSTEM_PROMPT = (() => {
        const { profile, relationLine } = profileCatalogue('learning-view-v1')
        const nodeLine = profile.nodeTypes.map((type) => type.id + ' ' + type.zh).join(' / ')
        return [
          '你是《学习观》知识图答疑审校员。用户会给出原文、知识图子图和具体质疑。原文是唯一事实源，不要凭常识修改作者观点。只输出合法 JSON。',
          '节点类型：' + nodeLine,
          '关系类型：' + relationLine,
          'verdict 只能为 supported（图有原文支持）、contradicted（图与原文矛盾）、insufficient（证据不足）或 out_of_scope（超出资料）。',
          'evidence 使用原文逐字摘录及本次给出的 [P数字] 段落编号。没有直接证据时给空数组。',
          '当 contradicted 或 insufficient 且可以确定一个非破坏性修复时，必须把修复编码为 proposedFix，不要只在 answer 中描述。一次只提一个最确定的修复；不能确定时用 {"action":"none"} 并解释原因。supported 和 out_of_scope 必须用 none。',
          '改节点类型或文字：{"action":"update_node","nodePatch":{"id":"现有节点ID","patch":{"type":"上列节点类型ID","text":"可选的修正文本"}}}。只改类型时不要改动原文 quote 或段落。',
          '提出 update_node 改型前，检查子图中该节点的全部入边和出边。新类型必须仍符合这些关系的 from/to 类型约束；若单一改型会使任何现有关系失效，返回 none 并在 answer 中指出需协同复核的关系。设问不是规律，也不能仅为避开规律误标而随意归入数据/经验。',
          '补遗漏节点：{"action":"add_node","nodePatch":{"patch":{"type":"上列节点类型ID","text":"从原文提炼的知识","quote":"原文逐字摘录","paragraph":2}}}。不得编造没有原文证据的材料。',
          '修关系：{"action":"update_edge|add_edge|delete_edge","edgePatch":{"fromNodeId":"旧边源节点ID","toNodeId":"旧边目标节点ID","relation":"新关系类型ID","evidence":[{"paragraph":2,"quote":"直接证明关系的原文逐字摘录"}]}}。update_edge 的 fromNodeId/toNodeId 必须定位一条已存在的旧边；改指向时另加 newFromNodeId 或 newToNodeId 指明新端点，不得把新目标写到旧边的 toNodeId。新增关系则 fromNodeId/toNodeId 直接指向新边两端。新增或修改关系必须有直接关系证据，方向和类型无法确定时用 none。',
          '提出 add_edge 或 update_edge 前核对新关系的 from/to 节点类型约束。例如 has_feature 的目标必须是 feature，不能是 feature_description；states_intension 的方向是 intension_description → concept。不能只因关系名字语义近似就建议一个类型不合法的边。',
          '只有节点本身没有原文依据、且无法通过 update_node 或关系修复时才可 delete_node；禁止因为缺少关系而删除仍有依据的节点。',
          'JSON 顶层结构：{"verdict":"contradicted","answer":"结论与解释","evidence":[{"paragraph":2,"quote":"原文逐字摘录"}],"proposedFix":{"action":"update_node","nodePatch":{"id":"n1","patch":{"type":"positive_example"}}}}。示例仅说明字段格式，实际类型必须来自上述本体。',
        ].join(NL)
      })()
      ...
      const CONSUMPTION_ANSWER_SYSTEM_PROMPT = [
        ...
        '',
        '硬性要求：',
        '1. status 只能取 answered / insufficient / out_of_scope。证据可以支持一个明确回答时用 answered；证据相关但不足以得出结论时用 insufficient；问题与检索到的资料无关时用 out_of_scope。只重复问题所引用的原句不算回答。',
        '2. answered 必须拆成 parts；每个 part 是一个简洁、自然、能独立核验的回答命题，并至少引用一个给定 evidenceId。先正面回答用户的问题，再补充必要的原因或例子。insufficient 也可用 parts 指出所引原句的具体歧义或当前证据的缺口，必须引用支持该判断的 evidenceId；不能因为一处引文未解释，就断言整份资料都未解释，更不得猜测作者本意或悄悄修正文句。part.text 必须复用证据中的关键名词或短语，不能只是复述整段引文；禁止把无关命题挂到一个真实 evidenceId 上。禁止自行填写 nodeId、paragraph、quote 或编造 evidenceId。Host 会逐句检查文本与引文，并要求独立语义复核。',
      ...
      const CONSUMPTION_ANSWER_REVIEW_SYSTEM_PROMPT = [
        ...
        '每个 part 只能从它所列的 evidenceId 中选择支持证据。answered 的第一条必须直接回答问题，不能只重复问题或引文；insufficient 的 part 只能准确说明原文歧义或证据缺口，不能猜测作者本意。',
      ...
      const SEMANTIC_GUARD_GROUPS = [
        ...
        ['只有'],
        ['如果', '一旦', '除非'],
        ['没有说明', '未说明', '没有证明', '未证明', '尚未验证', '未经验证', '尚不清楚', '证据不足', '未能证实', '没有证据', '未提供证据'],
      ]
      ...
      function applySafeInvariantRepairsHost(graph, validation, options = {}) {
        ...
        const allowSemanticStrengthRestore = options.allowSemanticStrengthRestore === true
        const repairs = []
        ...
        for (const issue of validation.blockingIssues || []) {
          ...
          } else if (allowSemanticStrengthRestore && issue.code === 'node_semantic_strength_drift' && issue.proposedFix && issue.proposedFix.nodePatch) {
            const patch = issue.proposedFix.nodePatch
            const node = nodes.find((item) => item && item.id === patch.id)
            const text = patch.patch && typeof patch.patch.text === 'string' ? patch.patch.text.trim() : ''
            if (node && text) {
              const previousText = node.text
              node.text = text
              refreshNodeGroundingStatusHost(node)
              repairs.push({ code: issue.code, targetId: node.id, action: 'restore_text_from_quote', previousText, text })
            }
      ...
      function invariantRepairSnapshotHost(graph, ontology = graph) {
        const candidate = graph && typeof graph === 'object' ? graph : {}
        const nodeAttributes = ontNodeAttributes(ontology), edgeAttributes = ontEdgeAttributes(ontology)
        return JSON.stringify({
          ...
          edges: (Array.isArray(candidate.edges) ? candidate.edges : []).map((edge) => ({
            ...
            ...(pickDeclaredAttributes(edge, edgeAttributes) || {}),
          })),
        })
      }
      ...
      function invariantRepairAttributeChangesHost(previous, candidate, ontology) {
        if (!previous) return []
        const nodeAttributes = ontNodeAttributes(ontology), edgeAttributes = ontEdgeAttributes(ontology)
        if (!nodeAttributes.size && !edgeAttributes.size) return []
        const changes = []
        const compare = (kind, id, before, after, allowed) => {
          const left = pickDeclaredAttributes(before, allowed) || {}, right = pickDeclaredAttributes(after, allowed) || {}
          const attributes = Array.from(allowed).filter(key => left[key] !== right[key])
          if (attributes.length && changes.length < 12) changes.push({ kind, id, attributes })
        }
        const nodes = new Map(candidate.nodes.map(node => [node.id, node]))
        for (const node of previous.nodes) compare('node', node.id, node, nodes.get(node.id), nodeAttributes)
        const edges = new Map(candidate.edges.map(edge => [edgeKeyHost(edge), edge]))
        const counts = new Map()
        for (const edge of previous.edges) counts.set(edgeKeyHost(edge), (counts.get(edgeKeyHost(edge)) || 0) + 1)
        for (const edge of previous.edges) {
          const key = edgeKeyHost(edge)
          // Invalid/duplicate edges may legitimately be removed or deduplicated.
          // An otherwise retained relation cannot silently change its role.
          if (counts.get(key) === 1 && edges.has(key)) compare('edge', key, edge, edges.get(key), edgeAttributes)
        }
        return changes
      }
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          for (const j of Array.from(candidates).sort((x, y) => x - y)) {
            ...
            if (sim >= 0.75 && a.node.type === b.node.type) {
              issue = { code: 'node_duplicate_suspected', category: 'duplicate',
                title: '疑似重复节点：' + a.node.id + ' / ' + b.node.id,
                detail: '两个同类型节点表述高度相似（相似度 ' + Math.round(sim * 100) + '%）。文字相似不能证明知识身份相同，请结合原文和全部关系人工复核。',
      ...
      async function buildLocalReportBatchedHost(graph, sourceText, task, sourceUnitLengths = task?.sourceUnitLengths) {
        const evaluated = validateGraphInvariantsHost(graph, sourceText, { includeQuality: true, skipPairChecks: true, sourceUnitLengths })
        ...
        evaluated.qualityIssues = evaluated.issues.filter((issue) => !issue.blocking)
        evaluated.metrics.qualityWarningCount = evaluated.qualityIssues.filter((issue) => issue.severity === 'warning').length
        Object.assign(evaluated.metrics, { checkedPairs, reviewBatches: batches, omittedPairIssues: omitted })
        const report = buildLocalReport(graph, sourceText, evaluated)
        if (omitted) report.summary += '另有 ' + omitted + ' 条相似性提示未展开，请缩小审校范围查看。'
        return report
      }

      function buildLocalReport(graph, sourceText, evaluated) {
        evaluated = evaluated || validateGraphInvariantsHost(graph, sourceText, { includeQuality: true })
        const issues = evaluated.issues.map((issue, index) => ({
          ...
          safeRepairable: issue.safeRepairable === true,
        }))
      ...
      function parseJson(raw) {
        ...
        try {
          ...
          // Syntactic completion can silently publish an unfinished graph.
          // Keep the original candidate intact for a complete retry instead.
      ...
      // A stream that produced nothing for the whole idle window is a provider
      // stall, not a verdict about the material. It still surfaces as a
      // `timeout`, and every stage treats that as terminal, so no outer retry
      // loop ever got a chance to run: one 180s silence could fail an entire
      // multi-hour extraction. Retry it once here, where the damage is bounded
      // to a single request and nothing has been committed yet.
      function isStreamStallHost(error) {
        return Boolean(error && error.code === 'timeout' && error.phase === 'llm_stream_idle')
      }
      ...
      async function callModel(model, system, userText, timeoutMs, temperature, userContent, timing) {
        ...
        // Display state contains request outcomes; never reuse it as the next request's label.
        const requestStage = timing?.stage || (task ? taskLabelHost(task.kind) : '模型请求')
        const stagePrefix = requestStage + '：'
        const requestProgress = { stage: requestStage, state: '准备请求', startedAt: Date.now(), lastContentAt: null, outputChars: 0, reasoningChars: 0 }
        if (task) {
      ...
      let pdfJsModuleHostPromise = null
      async function pdfJsModuleHost() {
        if (!pdfJsModuleHostPromise) pdfJsModuleHostPromise = import('pdfjs-dist/legacy/build/pdf.mjs')
        return pdfJsModuleHostPromise
      }
      async function pdfExtractTextPdfJsHost(buf) {
        const pdfjs = await pdfJsModuleHost()
        ...
        try {
          if (!Number.isInteger(doc.numPages) || doc.numPages < 1) return ''
          if (doc.numPages > MAX_PDF_PAGES_HOST) throw new Error('PDF 页数超过 ' + MAX_PDF_PAGES_HOST + ' 页安全上限')
          const pages = []
          let chars = 0
          for (let pageNo = 1; pageNo <= doc.numPages && chars < MAX_EXTRACTED_DOCUMENT_CHARS; pageNo++) {
            ...
            try {
              ...
              for (const item of content.items || []) {
                ...
                const x = item.transform && Number.isFinite(item.transform[4]) ? item.transform[4] : undefined
                const width = Number.isFinite(item.width) ? item.width : undefined
                if (line && prevX !== undefined && prevWidth !== undefined && x !== undefined && x > prevX + prevWidth + 1) line += ' '
                ...
                if (chars + line.length >= MAX_EXTRACTED_DOCUMENT_CHARS) break
              }
              if (line) lines.push(line)
              const pageText = lines.join(NL).trim()
              if (pageText) {
                const remaining = MAX_EXTRACTED_DOCUMENT_CHARS - chars
                pages.push(pageText.slice(0, remaining))
                chars += Math.min(pageText.length, remaining) + 2
              }
            } finally {
              try { page.cleanup() } catch (e) {}
            }
          }
          return pages.join(NL + NL).replace(/[ \t]+\n/g, NL).replace(/\n{3,}/g, NL + NL).trim().slice(0, MAX_EXTRACTED_DOCUMENT_CHARS)
        } finally {
      ...
      async function readDocumentBufferHost(buf, name, zlib) {
        const bytes = buf && Number.isInteger(buf.length) ? buf.length : 0
        if (bytes <= 0) return { error: '文件为空或无法读取' }
        if (bytes > MAX_DOCUMENT_FILE_BYTES) return { error: '文件超过 15 MiB，已跳过' }
        const lower = String(name || '').toLowerCase()
        if (lower.endsWith('.pdf') || (bytes >= 5 && buf.subarray(0, 5).toString('latin1') === '%PDF-')) {
          if (bytes < 5 || buf.subarray(0, 5).toString('latin1') !== '%PDF-') return { error: '文件扩展名是 PDF，但内容不是有效的 PDF 文件' }
          let text = ''
          let pdfJsError = null
          try { text = await pdfExtractTextPdfJsHost(buf) } catch (error) { pdfJsError = error }
          if (!text.trim()) text = pdfExtractTextHost(buf, zlib)
          if (text.trim()) return { text, format: 'pdf', bytes, warning: 'PDF 文本已通过 PDF.js 本地解析；复杂排版可能不完整，扫描版 PDF 暂不支持 OCR' }
          const detail = pdfJsError && pdfJsError.message ? '：' + pdfJsError.message : ''
          return { error: 'PDF 没有提取到文本（可能是扫描版、字体编码异常或超过解析安全上限）' + detail }
        }
        if (bytes >= 4 && buf[0] === 0x50 && buf[1] === 0x4b && (buf[2] === 3 || buf[2] === 5 || buf[2] === 7) && buf[3] < 10) {
          const text = zipExtractTextHost(buf, lower, zlib)
          return text && text.trim() ? { text, format: lower.split('.').pop() || 'zip', bytes } : { error: '压缩文档没有提取到文本（格式不支持或超过解压安全上限）' }
        }
        if (looksUtf8TextHost(buf, name)) {
          const text = buf.toString('utf8').replace(/^\uFEFF/, '').replace(/\r\n?/g, NL).slice(0, MAX_EXTRACTED_DOCUMENT_CHARS)
          if (text.trim()) return { text, format: 'text', bytes }
        }
      ...
      async function readDocumentTextHost(absPath, name) {
        ...
        try {
          ...
          return await readDocumentBufferHost(await handle.readFile(), name, mods.zlib)
        } finally {
          try { await handle.close() } catch (e) {}
        }
      }
      function decodeDirectDocumentBase64Host(value) {
        const input = typeof value === 'string' ? value : ''
        if (!input || input.length > Math.ceil(MAX_DOCUMENT_FILE_BYTES / 3) * 4 + 8 || !/^[A-Za-z0-9+/]+={0,2}$/.test(input)) return null
        try {
          const buf = Buffer.from(input, 'base64')
          const canonical = buf.toString('base64').replace(/=+$/, '')
          if (!buf.length || canonical !== input.replace(/=+$/, '')) return null
          return buf
        } catch (e) { return null }
      }
      async function collectDirectDocumentUploadsHost(uploads) {
        const mods = await docNodeModules()
        const found = []
        const warnings = []
        const list = Array.isArray(uploads) ? uploads.slice(0, MAX_DIRECT_DOCUMENT_UPLOADS_HOST) : []
        if (Array.isArray(uploads) && uploads.length > MAX_DIRECT_DOCUMENT_UPLOADS_HOST) warnings.push('一次最多上传 ' + MAX_DIRECT_DOCUMENT_UPLOADS_HOST + ' 个 PDF，其余文件已忽略')
        let totalBytes = 0
        for (let index = 0; index < list.length; index++) {
          const upload = list[index] && typeof list[index] === 'object' ? list[index] : {}
          const rawName = typeof upload.name === 'string' ? upload.name : ''
          const name = (rawName.split(/[\\/]/).pop() || ('文档-' + (index + 1) + '.pdf')).slice(0, 160)
          const mediaType = typeof upload.mediaType === 'string' ? upload.mediaType.toLowerCase() : ''
          if (!name.toLowerCase().endsWith('.pdf') && mediaType !== 'application/pdf') {
            warnings.push(name + '：直接上传目前仅支持 PDF 文件')
            continue
          }
          const buf = decodeDirectDocumentBase64Host(upload.data)
          if (!buf) {
            warnings.push(name + '：PDF 数据无效或超过 15 MiB')
            continue
          }
          if (buf.length < 5 || buf.subarray(0, 5).toString('latin1') !== '%PDF-') {
            warnings.push(name + '：文件内容不是有效的 PDF')
            continue
          }
          if (totalBytes + buf.length > MAX_DIRECT_DOCUMENT_TOTAL_BYTES_HOST) {
            warnings.push(name + '：PDF 总大小超过 20 MiB，已跳过')
            continue
          }
          totalBytes += buf.length
          const extracted = await readDocumentBufferHost(buf, name, mods.zlib)
          if (extracted.error) warnings.push(name + '：' + extracted.error)
          else found.push({ name, path: null, ...extracted })
        }
      ...
      function buildDocumentImportResultHost(collected) {
        let text = ''
        let remaining = MAX_TEXT
        let truncated = false
        const files = []
        const warnings = Array.isArray(collected.warnings) ? collected.warnings.slice() : []
        for (const f of collected.found) {
          ...
          if (f.warning) warnings.push(f.name + '：' + f.warning)
      ...
      function graphNodeLookupKeyHost(node) {
        // Search normalization is deliberately lossy; identity must retain
        // punctuation, numeric separators, case and assertion context.
        const text = String(node && node.text || '').normalize('NFC').replace(/\s+/g, ' ').trim()
        if (!text) return ''
        const type = node && node.type || ''
        const deictic = /^(?:其|它|该|此|本|上述|前者|后者|this\b|that\b|it\b)/i.test(text)
        if (type === 'concept' && !deictic) return JSON.stringify([type, text])
        if (!Number.isInteger(node.paragraph)) return ''
        return JSON.stringify([type, text, node.documentId || '', node.sourceId || '', node.sectionId || node.sectionTitle || '', node.paragraph])
      }
      ...
      function serializeExistingGraph(existing, maxNodes, queryText, cache = new Map()) {
        ...
        for (let index = 0; index < nodes.length; index++) {
          ...
          const text = typeof n.text === 'string' ? n.text.trim() : ''
          if (!id || !text) continue
          let indexed = cache.get(id)
          if (!indexed || indexed.text !== text) {
            const normalized = normalizeGraphLookupTextHost(text)
            indexed = { text, normalized, tokens: phraseTokensHost(normalized) }
            cache.set(id, indexed)
          }
        ...
        // Equal lexical matches prefer recent context instead of repeatedly
        // supplying the first 24 nodes of an ever-growing document.
        if (queryTokens.size > 0) candidates.sort((a, b) => b.score - a.score || b.index - a.index)
      ...
      function shouldWeaveRelationsHost(stats) {
        if (!stats || stats.nodeCount < 2) return false
        if (stats.nodeCount === 2) return stats.edgeCount === 0
      ...
      function relationParagraphDistanceHost(a, b, stats) {
        const paragraphsFor = node => stats.lookupParagraphs?.get(node.id) || (Number.isInteger(node.paragraph) ? [node.paragraph] : [])
        let distance = Infinity
        for (const pa of paragraphsFor(a)) for (const pb of paragraphsFor(b)) distance = Math.min(distance, Math.abs(pa - pb))
        return distance
      }
      function relationCandidateScoreHost(a, b, stats, distance = relationParagraphDistanceHost(a, b, stats)) {
        let score = 0
        score += distance === 0 ? 4 : (distance <= 8 ? 3 / distance : 0)
        if (a && b && a.sectionId && a.sectionId === b.sectionId) score += 2.5
        if (!stats.lookupTokens) stats.lookupTokens = new Map()
        const tokensFor = (node) => {
          if (!stats.lookupTokens.has(node.id)) stats.lookupTokens.set(node.id, phraseTokensHost(normalizeGraphLookupTextHost(node.text)))
          return stats.lookupTokens.get(node.id)
        }
        const at = tokensFor(a)
        const bt = tokensFor(b)
      ...
      function buildRelationWeaveGroupsHost(nodes, stats, paragraphTexts, previousCoverage, sourceText) {
        ...
        stats.lookupParagraphs = new Map()
        for (const node of nodes) {
          ...
          // Deduped concepts retain later occurrences in evidence. Index all
          // anchors so later chapters can still find their canonical concept.
          const paragraphs = relationNodeParagraphsHost(node, paragraphTexts)
          stats.lookupParagraphs.set(node.id, paragraphs)
          for (const paragraph of paragraphs) {
            if (!byParagraph.has(paragraph)) byParagraph.set(paragraph, [])
            byParagraph.get(paragraph).push(node.id)
          }
        }
      ...
      function relationContextRecordsHost(nodes, paragraphTexts, ontology = null) {
      ...
      function prepareRelationWeaveContextsHost(groups, paragraphTexts, ontology = null) {
        ...
        while (entries.length) {
          ...
          first.records = first.records || relationContextRecordsHost(first.nodes, paragraphTexts, ontology)
          ...
          for (let i = 0; i < Math.min(entries.length, 8); i++) {
            entries[i].records = entries[i].records || relationContextRecordsHost(entries[i].nodes, paragraphTexts, ontology)
      ...
      function buildRelationWeaveUserTextHost(title, groupNodes, existingEdges, paragraphTexts, stats, index, total, targetIds = [], context = null, ontology = null) {
        ...
        text += (context ? context.sharedContext : Array.from(relationContextRecordsHost(groupNodes, paragraphTexts, ontology).values()).join(NL)) + NL
        ...
        for (const edge of existingEdges) {
          if (!ids.has(edge.fromNodeId) || !ids.has(edge.toNodeId)) continue
          const attributes = pickDeclaredAttributes(edge, ontEdgeAttributes(ontology))
          text += edge.fromNodeId + '>' + edge.toNodeId + ':' + edge.relation + (attributes ? '|' + JSON.stringify(attributes) : '') + NL
          listed += 1
          if (listed >= 240) break
        }
      ...
      async function weaveRelationsHost(task, model, acc, paragraphTexts, sourceInfo, sourceText, previousCoverage = null) {
        ...
        groups = prepareRelationWeaveContextsHost(groups, paragraphTexts, task)
        ...
        // Keep candidates separate from the extraction graph until independent
        // review. Bind cached groups to the original plan, not the evolving graph.
        const journalVersion = task.relationWeave?.version ?? 2
        const binding = sha256HexHost(JSON.stringify({
          policy: journalVersion === 1 ? 'relation-weave-v1' : 'relation-weave-v2', ontology: ontIdOf(task), sourceText,
        ...
        if (![1, 2].includes(journal.version) || journal.binding !== binding || journal.totalGroups !== groups.length || !journal.results || typeof journal.results !== 'object' || Array.isArray(journal.results)) throw invalidJournal()
        for (const [key, cached] of Object.entries(journal.results)) {
          const index = Number(key)
          const ids = new Set(groups[index]?.nodes.map(node => node.id))
          if (!/^(0|[1-9]\d*)$/.test(key) || index >= groups.length || !cached?.norm || !Array.isArray(cached.norm.edges) || !Array.isArray(cached.norm.warnings) || cached.hash !== sha256HexHost(JSON.stringify(cached.norm))) throw invalidJournal()
          if (cached.norm.edges.some(edge => !ids.has(edge.fromNodeId) || !ids.has(edge.toNodeId))) throw invalidJournal()
        ...
        await runRelationQueueHost(task, queue, '关系审校', async (entries, save, check) => {
          ...
          for (let attempt = 0; attempt < 2; attempt++) {
            ...
            for (const verdict of parsed.verdicts) {
              if (!verdict || !ids.has(verdict.id)) throw new Error('review_unknown_candidate_id')
              if (verdicts.has(verdict.id)) throw new Error('review_duplicate_candidate_id')
              if (!['supported', 'contradicted', 'insufficient'].includes(verdict.verdict)) throw new Error('review_invalid_verdict_enum')
              const evidence = normalizeRelationEvidenceHost(verdict.evidence, paragraphTexts.length, { paragraphTexts }, null, verdict.id)
              if (evidence.some((item) => !numbers.has(item.paragraph))) throw new Error('review_evidence_outside_batch')
              if (verdict.verdict !== 'insufficient' && !evidence.length) throw new Error('review_missing_authenticated_evidence')
      ...
      function taskLabelHost(kind) {
        const labels = { extract: '资料拆分', resume: '拆分续跑', append: '追加拆分', 'relation-retry': '关系检索', verify: 'AI 深度审校', question: '节点答疑', 'fact-check': '外部核查', trajectory: '轨迹拆分', 'trajectory-append': '轨迹追加', answer: '知识图答疑' }
        return labels[kind || 'extract'] || 'AI 任务'
      }
      function taskDescriptorHost(task) {
        if (!task) return null
        const kind = task.kind || 'extract'
        return {
          taskId: task.id, kind, label: taskLabelHost(kind), status: taskStateHost(task),
      ...
      function extractionWaveContextHashHost(task, acc, sourceManifest, version = 2) {
        const snapshot = invariantRepairSnapshotHost({ nodes: Array.from(acc.nodes.values()), edges: acc.edges }, version === 1 ? DEFAULT_ONTOLOGY : task)
        // Read old checkpoints with their original codec; only new waves use
        // the ontology-aware binding. Never silently reinterpret saved hashes.
        return sha256HexHost(version === 1 ? sourceManifest.sourceId + snapshot : JSON.stringify({
          policy: 'wave-context-v2', ontology: ontIdOf(task), sourceId: sourceManifest.sourceId, graph: snapshot,
        }))
      }
      /**
        * Why a restored concurrent wave can no longer be trusted, or null when it
        * still describes the accumulator. Two callers need this same verdict: the
        * resume path checks it BEFORE rebuilding and persisting the checkpoint,
        * and the wave loop checks it again before replaying the wave. Keeping one
        * predicate is what stops the two from drifting apart.
        */
      function pendingWaveMismatchHost(task, waveStart, batches, acc, sourceManifest) {
        const wave = task && task.pendingWave
        if (!wave) return null
        const version = wave.contextVersion ?? 1
        if (![1, 2].includes(version)) return '无法识别波次语义上下文版本'
        const contextHash = extractionWaveContextHashHost(task, acc, sourceManifest, version)
        if (wave.start !== waveStart) return 'wave.start=' + wave.start + ' 应为 ' + waveStart
        if (!Number.isInteger(wave.end) || wave.end <= waveStart || wave.end > Math.min(batches.length, waveStart + 4)) return 'wave.end=' + wave.end
        if (wave.contextHash !== contextHash) return '已合并图与波次记录不一致'
        if (!wave.results || typeof wave.results !== 'object' || Array.isArray(wave.results)) return 'wave.results 非法'
        const badKey = Object.keys(wave.results).find((key) => !/^\d+$/.test(key) || Number(key) < wave.start || Number(key) >= wave.end)
        if (badKey !== undefined) return 'wave.results 键越界：' + badKey
        return null
      }
      ...
      async function saveTaskCheckpointHost(task, checkpoint, status = 'running') {
        if (typeof persistCheckpoint === 'function') {
          try { await persistCheckpoint(checkpoint, task, status) }
          catch (error) { throw taskOperationErrorHost('persistence_failed', '检查点保存失败：' + (error?.message || error), 'checkpoint') }
        }
      ...
      // Checkpoints contain only owned JSON data. They are returned on demand
       ...
       function buildTaskCheckpoint(task, sourceManifest, chunkResults, acc, summary, nextBatchIndex) {
         ...
         return {
             version: 2,
             batchPolicy: sourceManifest.batchPolicy,
           ...
           ...(task.relationWeave ? { relationWeave: JSON.parse(JSON.stringify(task.relationWeave)) } : {}),
      ...
      async function runTask(task) {
        ...
        try {
          ...
          task.relationWeave = isResume && task.checkpoint?.relationWeave ? JSON.parse(JSON.stringify(task.checkpoint.relationWeave)) : null
          ...
          if (task.relationWeave && task.checkpoint.nextBatchIndex !== batches.length) return failTask(task, 'checkpoint_invalid', '关系补全检查点必须基于已完成的正文拆分')
          ...
          const sourceContext = {
           ...
           const acc = { nodes: new Map(), edges: [], edgeKeys: new Set(), warnings: [], nodeKeys: new Map(), lookupTokens: new Map() }
           if (!isResume && task.imageSource) {
             await saveTaskCheckpointHost(task, buildTaskCheckpoint(task, sourceManifest, [], acc, '', 0))
           }
          let summary = ''
          ...
          if (existing) {
            ...
            const storedNodeAttributes = ontNodeAttributes(task)
            const storedEdgeAttributes = ontEdgeAttributes(task)
            ...
            for (const n of existing.nodes || []) {
              ...
              const seededNode = {
                ...
                text,
                ...(pickDeclaredAttributes(n, storedNodeAttributes) || {}),
            ...
            for (const e of existing.edges || []) {
              ...
              acc.edges.push({
                 ...
                 toNodeId: to,
                 relation: rel,
                 ...(pickDeclaredAttributes(e, storedEdgeAttributes) || {}),
          ...
          // Validate a restored wave BEFORE the rebuilt accumulator is written
          // over the checkpoint. The old order persisted first and only then
          // discovered the mismatch, so a failed resume replaced the only copy of
          // the original state with the damaged rebuild and the run became
          // unrecoverable instead of merely failed. Failing here leaves
          // `task.checkpoint` as the checkpoint we loaded, which is what the
          // failure path then persists back unchanged.
          if (isResume && task.pendingWave) {
            const mismatch = pendingWaveMismatchHost(task, resumeFromBatch, batches, acc, sourceManifest)
            if (mismatch) return failTask(task, 'checkpoint_invalid', '并发检查点与已合并图不一致，禁止跳过批次（' + mismatch + '）；原检查点已保留')
          }
          await saveTaskCheckpointHost(task, buildTaskCheckpoint(task, sourceManifest, chunkResults, acc, summary, resumeFromBatch))
          ...
          const extractBatch = async (i, prepared, saveBatchResult) => {
            ...
            for (let attempt = 0; !norm && attempt < 3; attempt++) {
              try {
                ...
                const changedAttributes = invariantRepairAttributeChangesHost(repairReference, r, task)
                if (changedAttributes.length) {
                  const feedback = 'repair_semantic_attributes_changed：修复候选删除或改写了未被要求修改的本体属性。必须保留完整候选中的节点 id 和属性，不得在修复其他 invariant 时改变材料阶段或关系角色。变更项=' + JSON.stringify(changedAttributes)
                  repairFeedback = feedback + (repairFeedback ? NL + repairFeedback : '')
                  lastFailureCode = 'invariant_violation'
                  lastErr = feedback
                  if (attempt < 2) {
                    generationInvariantRetries += 1
                    taskStage('第 ' + (i + 1) + '/' + batches.length + ' 个内容块修复时改变了本体属性，正在要求完整重试…')
                  }
                  continue
                }
                ...
                if (gate.blockingIssues.length > 0 && attempt < 2) {
                  ...
                  repairReference = JSON.parse(repairSnapshot)
                ...
                if (gate.blockingIssues.length > 0) {
                  ...
                  const fallback = applySafeInvariantRepairsHost(r, gate, { allowEdgeDrops: true, allowSemanticStrengthRestore: true })
          ...
          for (let waveStart = resumeFromBatch; waveStart < batches.length;) {
            ...
            const wave = task.pendingWave || { start: waveStart, end: Math.min(batches.length, waveStart + task.concurrency),
              contextVersion: 2, contextHash: extractionWaveContextHashHost(task, acc, sourceManifest), results: {} }
            const mismatch = task.pendingWave ? pendingWaveMismatchHost(task, waveStart, batches, acc, sourceManifest) : null
            if (mismatch) {
              return failTask(task, 'checkpoint_invalid', '并发检查点与已合并图不一致，禁止跳过批次（' + mismatch + '）')
            }
            task.pendingWave = wave
            let persistence = Promise.resolve()
            const saveWave = () => {
              const snapshot = buildTaskCheckpoint(task, sourceManifest, chunkResults, acc, summary, waveStart)
              persistence = persistence.then(() => saveTaskCheckpointHost(task, snapshot))
              return persistence
            }
            const updateSavedCounts = () => {
              const saved = Object.values(task.checkpoint?.pendingWave?.results || {})
              task.progress.parallel.saved = saved.filter(item => item.stage !== 'coverage_pending').length
              task.progress.parallel.prepared = saved.filter(item => item.stage === 'coverage_pending').length
            }
          ...
          const sourceSections = sourceManifest.sections.map((section) => {
           ...
           if (task.postprocess) {
             ...
             try {
               ...
               if (isTerminalTaskOperationErrorHost(error) || ['persistence_failed', 'checkpoint_invalid'].includes(error?.code)) throw error
               const connectivity = graphConnectivityHost(nodes, acc.edges)
          ...
          const fullResult = task.postprocess ? JSON.parse(JSON.stringify(task.postprocess.graph)) : {
           ...
           for (const node of fullResult.nodes) {
             if (Number.isInteger(node.paragraph)) anchoredUnits.add(node.paragraph)
             for (const evidence of node.evidence || []) if (Number.isInteger(evidence.paragraph)) anchoredUnits.add(evidence.paragraph)
           }
           const unassessedRanges = []
           for (let paragraph = 0; paragraph < canonicalParagraphTexts.length; paragraph++) {
             if (anchoredUnits.has(paragraph)) continue
             const last = unassessedRanges[unassessedRanges.length - 1]
             if (last && last.end + 1 === paragraph) last.end = paragraph
           ...
           fullResult.generation = {
             ...
             coverage: {
               sourceUnits: canonicalParagraphTexts.length,
               anchoredUnits: Array.from(anchoredUnits).filter((p) => p >= 0 && p < canonicalParagraphTexts.length).length,
               unassessedRanges,
               // An anchor is not proof that all propositions were extracted.
           ...
           if (typeof persistGraph === 'function') {
             try {
               const canonicalSourceUnits = splitParagraphsHost(canonicalSourceText)
                const persisted = await persistGraph(fullResult, { ...task, checkpoint: completedCheckpoint, canonicalSourceText, canonicalSourceUnits })
           ...
           task.checkpoint = completedCheckpoint
      ...
      function consumptionEvidenceSupportsPartHost(partText, evidence, authorityText = partText, diagnostic = false) {
        ...
        return overlap >= 2 && (partCoverage >= (diagnostic ? 0.2 : 0.35) || evidenceCoverage >= 0.45)
      }
      function consumptionSupportedEvidenceIdsHost(partText, evidenceIds, evidenceById, diagnostic = false) {
        const clauses = String(partText || '').split(/[。！？；\n]+/).map((item) => item.trim()).filter(Boolean)
        if (clauses.length === 0) return []
        const used = new Set()
        for (const clause of clauses) {
          const supporting = evidenceIds.filter((id) => consumptionEvidenceSupportsPartHost(clause, evidenceById.get(id), '根据这份资料，' + clause, diagnostic))
      ...
      function consumptionAnswerCandidatesHost(raw, evidenceCatalog) {
        ...
        const candidates = []
        const parts = raw && ['answered', 'insufficient'].includes(raw.status) && Array.isArray(raw.parts) ? raw.parts : []
        for (let index = 0; index < parts.length && candidates.length < 3; index++) {
          ...
          const evidenceIds = consumptionSupportedEvidenceIdsHost(text, ids, evidenceById, raw.status === 'insufficient')
      ...
      function consumptionRestatesQuotedQuestionHost(question, answer) {
        if (!/^(为什么|为何|怎么|如何|何以)/.test(String(question || '').trim())) return false
        const quoted = String(question || '').match(/[“「"]([^”」"]{8,})[”」"]/)?.[1]
        if (!quoted) return false
        const compact = (value) => normalizeGraphLookupTextHost(value).replace(/[^\p{L}\p{N}]/gu, '')
        const claim = compact(quoted)
        const text = compact(answer)
        return claim.length >= 8 && text.includes(claim) && text.length - claim.length <= 10
      }
      function consumptionQuotedEvidenceHost(question, evidenceCatalog) {
        const quoted = String(question || '').match(/[“「"]([^”」"]{8,})[”」"]/)?.[1]
        if (!quoted) return null
        const compact = (value) => normalizeGraphLookupTextHost(value).replace(/[^\p{L}\p{N}]/gu, '')
        const claim = compact(quoted)
        if (claim.length < 8) return null
        return (Array.isArray(evidenceCatalog) ? evidenceCatalog : []).find((item) => compact(item.quote).includes(claim)) || null
      }
      function consumptionOverlappingContrastHost(question, evidenceCatalog) {
        const evidence = consumptionQuotedEvidenceHost(question, evidenceCatalog)
        if (!evidence) return null
        const match = String(evidence.quote || '').match(/关于([^，。；]{2,32}?)的概念[，,]\s*而非([^，。；]{2,32}?)的概念/)
        if (!match) return null
        const left = match[1].trim()
        const right = match[2].trim()
        if (!left || !right || !(left.endsWith(right) || right.endsWith(left))) return null
        return { evidence, left, right }
      }
      ...
      function normalizeConsumptionAnswerHost(raw, evidenceCatalog, review = new Map(), question = '') {
        ...
        let directAnswerApproved = false
        for (const candidate of candidates) {
          ...
          const supportedIds = consumptionSupportedEvidenceIdsHost(candidate.text, reviewedIds, evidenceById, status === 'insufficient')
          const natural = supportedIds.length > 0 && !(status === 'answered' && consumptionRestatesQuotedQuestionHost(question, candidate.text))
          if (candidate.id === 'draft-0' && status === 'answered' && natural) directAnswerApproved = true
        ...
        if (status === 'answered' && !directAnswerApproved) {
          status = 'insufficient'
          confidence = Math.min(confidence, 0.35)
          parts = parts.slice(0, 1)
        }
        if (status === 'insufficient') parts = parts.slice(0, 1)
        if (status === 'insufficient' && parts.length === 0) {
          const quotedEvidence = consumptionQuotedEvidenceHost(question, evidenceCatalog)
          if (quotedEvidence) parts.push({ id: 'part-1', text: quotedEvidence.quote, evidenceIds: [quotedEvidence.evidenceId], mode: 'extractive' })
        }
        const ambiguity = status === 'insufficient' ? consumptionOverlappingContrastHost(question, evidenceCatalog) : null
        if (ambiguity) parts = [{ id: 'part-1', text: ambiguity.evidence.quote, evidenceIds: [ambiguity.evidence.evidenceId], mode: 'extractive' }]
        if (status === 'insufficient') confidence = Math.min(confidence, 0.35)
        const usedEvidenceIds = new Set(parts.flatMap((part) => part.evidenceIds))
        let answer = ambiguity
          ? '原文第 ' + ambiguity.evidence.paragraph + ' 段确实这样写，但它把“' + ambiguity.left + '”与“' + ambiguity.right + '”作为对立项，措辞重叠；仅凭这句话无法说明两者为何不同。不能把复述原句当作解释，也不能擅自断定作者原意。'
          : parts.length > 0
          ? status === 'answered'
            ? '根据这份资料：' + NL + parts.map((item) => item.text).join(NL + NL)
            : parts.some((item) => item.mode === 'natural')
              ? parts.map((item) => item.text).join(NL + NL)
              : '找到了相关原文，但它本身尚未解释这个问题：' + NL + parts.map((item) => item.text).join(NL + NL)
          : ''
      ...
      async function runConsumptionAnswerTask(task) {
        ...
        try {
          ...
          const answer = normalizeConsumptionAnswerHost(parsed, evidenceCatalog, review, task.question)
          task.status = 'succeeded'
          task.finishedAt = Date.now()
          task.result = {
            ...answer,
      ...
      function verificationTaskMetadataHost(args, graph) {
        const clean = (value, limit) => typeof value === 'string' ? value.trim().slice(0, limit) : ''
        return {
          title: clean(args.title, 200) || clean(graph?.source?.title, 200),
          documentId: clean(args.documentId, 160) || clean(graph?.source?.documentId, 160),
        }
      }
      function reviewSourceParagraphsHost(graph, issue) {
        ...
        for (const node of Array.isArray(graph?.nodes) ? graph.nodes : []) { add(node?.paragraph); evidence(node?.evidence) }
        for (const edge of Array.isArray(graph?.edges) ? graph.edges : []) evidence(edge?.evidence)
        evidence(issue?.evidence)
        return required
      }
      function verificationParagraphsHost(sourceText, sourceUnitLengths) {
        if (sourceUnitLengths === undefined) return splitParagraphsOffsetsHost(sourceText || '')
        const invalid = () => taskOperationErrorHost('source_units_invalid', '原文段落边界与审校输入不一致，禁止继续使用错位证据', 'verification')
        if (!Array.isArray(sourceUnitLengths) || !sourceUnitLengths.length || sourceUnitLengths.length > MAX_VERIFY_SCOPE_UNITS) throw invalid()
        const paragraphs = []
        let start = 0, chars = 0
        for (const length of sourceUnitLengths) {
          if (!Number.isSafeInteger(length) || length <= 0 || (chars += length) > MAX_VERIFY_SCOPE_CHARS) throw invalid()
          const end = start + length
          if (end > sourceText.length || (start > 0 && sourceText.slice(start - 2, start) !== NL + NL)) throw invalid()
          const text = sourceText.slice(start, end)
          if (!text.trim()) throw invalid()
          paragraphs.push({ text, start, end })
          start = end + 2
        }
        if (start - 2 !== sourceText.length) throw invalid()
        return paragraphs
      }
      function prepareVerificationInputHost(args, requireCompleteSource = false) {
        const rawText = typeof args.text === 'string' && args.text.trim() ? args.text : ''
        ...
        const required = requireCompleteSource ? reviewSourceParagraphsHost(graph, args.reviewIssue) : new Set()
        const incomplete = () => ({ error: { code: 'review_source_incomplete',
          message: '复核必需的原文段落缺失或超过单次上下文上限；请核对原文或拆分该问题后重试，未调用模型' } })
        if (requireCompleteSource && args.reviewIssue?.targetKind === 'graph'
          && (sourceUnits.length > 0 || !rawText || rawText.length > MAX_VERIFY_SCOPE_CHARS
            || splitParagraphsOffsetsHost(rawText).length > MAX_VERIFY_SCOPE_UNITS)) {
          return { error: { code: 'review_source_incomplete',
            message: '整图问题必须核对完整原文，不能使用原文片段或超过单次 AI 复核上限；请将问题缩小到具体节点或关系，未调用模型' } }
        }
        if (!graph || sourceUnits.length === 0) {
          if (requireCompleteSource && graph) {
            const paragraphs = splitParagraphsOffsetsHost(rawText)
            if (required.size > MAX_VERIFY_SCOPE_UNITS || [...required].some(paragraph => !paragraphs[paragraph]?.text?.trim())
              || [...required].reduce((sum, paragraph) => sum + paragraphs[paragraph].text.length, 0) > MAX_VERIFY_SCOPE_CHARS) return incomplete()
          }
          return { text: rawText, graph, paragraphMap: null, scoped: false }
        }
        const units = []
        const seen = new Set()
        let chars = 0
        const candidates = requireCompleteSource
          ? [...sourceUnits.filter(unit => required.has(Number(unit?.paragraph))), ...sourceUnits.filter(unit => !required.has(Number(unit?.paragraph)))]
          : sourceUnits
        for (const unit of candidates) {
          if (!unit || typeof unit !== 'object') continue
          const paragraph = Number(unit.paragraph)
          const text = typeof unit.text === 'string' ? unit.text : ''
          if (!Number.isInteger(paragraph) || paragraph < 0 || !text.trim() || seen.has(paragraph)) continue
          if (units.length >= MAX_VERIFY_SCOPE_UNITS || chars + text.length > MAX_VERIFY_SCOPE_CHARS) break
          seen.add(paragraph)
          units.push({ paragraph, text })
          chars += text.length
        }
        if (requireCompleteSource && (units.length === 0 || [...required].some(paragraph => !seen.has(paragraph)))) return incomplete()
        ...
        const scopedNodes = (Array.isArray(graph.nodes) ? graph.nodes : [])
          .filter((node) => {
            if (!node || typeof node !== 'object') return false
            if (requireCompleteSource) return true
        ...
        return {
          ...
          graph: scopedGraph,
          paragraphMap,
          // Units already carry canonical identity; joined text is not a new
          // document to segment again. Keep boundaries through task recovery.
          sourceUnitLengths: units.map(unit => unit.text.length),
          scoped: true,
        }
      }
      ...
      function serializeGraphForVerify(graph, nodeIds) {
        ...
        const edges = (graph.edges || [])
          ...
          .map((e) => ({ fromNodeId: e.fromNodeId, toNodeId: e.toNodeId, relation: e.relation, evidence: cloneEvidenceHost(e.evidence) }))
      ...
      function buildVerifyBatches(paras, graph) {
        ...
        for (const b of batches) {
          ...
          for (let i = 0; i < nodes.length; i += 12) {
            final.push({ units: b.units, pSet: b.pSet, nodes: nodes.slice(i, i + 12) })
          }
        }
        // Edges crossing node batches would otherwise never appear in a
        // judge request. Review them with both endpoints and their evidence.
        const reviewed = new Set()
        for (const batch of final) {
          const ids = new Set(batch.nodes.map((node) => node.id))
          for (const edge of graph.edges || []) if (ids.has(edge.fromNodeId) && ids.has(edge.toNodeId)) reviewed.add(edgeKeyHost(edge))
        }
        const crossEdges = (graph.edges || []).filter((edge) => !reviewed.has(edgeKeyHost(edge)))
        const byId = new Map((graph.nodes || []).map((node) => [node.id, node]))
        for (let i = 0; i < crossEdges.length; i += 12) {
          const edges = crossEdges.slice(i, i + 12)
          ...
          const paragraphs = new Set(nodes.map((node) => node.paragraph))
          for (const item of [...nodes, ...edges]) for (const evidence of item.evidence || []) paragraphs.add(evidence.paragraph)
          const units = Array.from(paragraphs).filter((p) => Number.isInteger(p) && p >= 0 && p < paras.length).sort((a, b) => a - b).map((num) => ({ num, text: paras[num] }))
          final.push({ units, pSet: new Set(units.map((unit) => unit.num)), nodes, edges, relationsOnly: true })
        }
        return final
      }
      ...
      function sanitizeFix(fix, graph, sourceText, totalParagraphs, sourceUnitLengths) {
        ...
        const completeText = (value, limit) => {
        ...
        const cleanNodePatch = (p) => {
          ...
          // Clipping a repair can remove a negation or qualifier. Reject the whole
          // proposal instead of applying a different claim or a partial patch.
          for (const [field, limit] of [['text', MAX_FIX_TEXT_CHARS], ['quote', MAX_FIX_QUOTE_CHARS]]) {
            if (typeof patch[field] !== 'string') continue
            const text = completeText(patch[field], limit)
        ...
        const cleanEdgePatch = (p, action) => {
          ...
          const newFrom = action === 'update_edge' && typeof p.newFromNodeId === 'string' ? p.newFromNodeId : from
          const newTo = action === 'update_edge' && typeof p.newToNodeId === 'string' ? p.newToNodeId : to
          if (!ids.has(newFrom) || !ids.has(newTo) || newFrom === newTo) return null
          const oldRelation = typeof p.oldRelation === 'string'
            ? ontRelationAliases(graph)[p.oldRelation.trim().toLowerCase()] : null
          if (p.oldRelation && !oldRelation) return null
          const oldEdges = action === 'update_edge' || action === 'delete_edge'
            ? (graph.edges || []).filter((edge) => edge.fromNodeId === from && edge.toNodeId === to
              && (action === 'delete_edge' ? edge.relation === relation : (!oldRelation || edge.relation === oldRelation))) : []
          if ((action === 'update_edge' || action === 'delete_edge') && oldEdges.length !== 1) return null
          if (action !== 'delete_edge') {
            const rule = (ontProfile(graph).relationTypes || []).find((entry) => entry.id === relation)
            const fromType = nodes.find((node) => node.id === newFrom)?.type
            const toType = nodes.find((node) => node.id === newTo)?.type
            if (!rule || (Array.isArray(rule.from) && !rule.from.includes(fromType))
              || (Array.isArray(rule.to) && !rule.to.includes(toType))) return null
          }
          const out = { fromNodeId: from, toNodeId: to, relation }
          if (action === 'update_edge' && newFrom !== from) out.newFromNodeId = newFrom
          if (action === 'update_edge' && newTo !== to) out.newToNodeId = newTo
          if (oldEdges.length === 1) out.oldRelation = oldEdges[0].relation
          if (typeof p.index === 'number' && p.index >= 0) out.index = p.index
          const relationEvidence = normalizeRelationEvidenceHost(
            p.evidence,
            totalParagraphs,
            { paragraphTexts: verificationParagraphsHost(sourceText || '', sourceUnitLengths).map(paragraph => paragraph.text) },
            null,
            newFrom + '->' + newTo + ':' + relation,
          )
        ...
        if (action === 'update_node' || action === 'delete_node') {
          ...
          const p = cleanEdgePatch(fix.edgePatch, action)
          ...
          const text = completeText(typeof fix.summaryPatch === 'string' && fix.summaryPatch.trim()
            ? fix.summaryPatch : fix.nodePatch?.patch?.text, MAX_FIX_TEXT_CHARS)
          if (!text) return { action: 'none' }
          clean.summaryPatch = text
        }
        return clean
      }
      function normalizeIssues(obj, graph, sourceText, totalParagraphs, warnings, idPrefix, sourceUnitLengths) {
        const issues = []
        const seen = new Set()
        for (const raw of Array.isArray(obj && obj.issues) ? obj.issues : []) {
          ...
          issues.push({
            ...
            evidence,
            confidence,
            proposedFix: sanitizeFix(raw.proposedFix, graph, sourceText, totalParagraphs, sourceUnitLengths),
            status: 'open',
          })
        }
        return { issues }
      ...
      async function callVerificationJsonHost(task, model, systemPrompt, userText, timeoutMs, stage, resultKey, batchProgress, check, candidateIds) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          check()
          Object.assign(batchProgress, { attempt: attempt + 1, maxAttempts: 3 })
          try {
            const raw = await callModel(model, systemPrompt, userText, timeoutMs, 0.1, null,
              { stage: stage + (attempt ? '（重试 ' + attempt + '/2）' : '') })
            check()
            const result = raw && typeof raw === 'object' ? raw : parseJson(raw)
            if (!Array.isArray(result?.[resultKey])) throw new Error('审校 JSON 缺少 ' + resultKey + ' 数组')
            if (resultKey === 'kept' && result.kept.some(item => !item || typeof item.id !== 'string' || !candidateIds.has(item.id))) {
              throw new Error('独立复核返回了无效或不属于本批的候选编号')
            }
            return result
          } catch (error) {
            check()
            if (isTerminalTaskOperationErrorHost(error)) throw error
            if (Number.isInteger(error.status) && error.status >= 400 && error.status < 500 && ![408, 429].includes(error.status)) throw error
            lastError = error
            if (isRelationRateLimitErrorHost(error)) {
              task.relationConcurrencyLimit = 1
              task.progress.warning = '模型服务限流，后续审校降为 1 路并发，等待后重试'
              if (task.progress.relationParallel) task.progress.relationParallel.limit = 1
            }
            const delay = relationReviewRetryDelayHost(error)
            if (attempt < 2 && delay) await cancellableTaskDelayHost(task, delay)
          }
        }
        throw taskOperationErrorHost('verification_failed', stage + '失败（已尝试 3 次）：' + (lastError?.message || String(lastError)), 'verification')
      }
      ...
      async function runVerifyTask(task) {
        ...
        task.progress = { stage: '准备审校', charsReceived: 0, updatedAt: Date.now(),
          verification: { phase: 'preparing', totalBatches: 0, completedBatches: 0, checkedPairs: 0, activeBatches: [] } }
        activeTask = task
        try {
          ...
          task.model = model
          if (model) announceModel(task, model)
          const paras = verificationParagraphsHost(task.text, task.sourceUnitLengths).map(paragraph => paragraph.text)
              ...
              for (let scopeAttempt = 0; scopeAttempt < (fullPlan ? 2 : 1); scopeAttempt++) {
                ...
                try {
                  ...
                  const candidateIssues = normalizeIssues(obj, task.graph, task.text, totalParagraphs,
                    candidateWarnings, 'b' + (i + 1) + ':', task.sourceUnitLengths).issues
                  if (fullPlan) {
      ...
      function buildQuestionContext(graph, sourceText, target, question = '', paragraphMap = null, sourceUnitLengths) {
        const paras = sourceUnitLengths === undefined ? splitParagraphsOffsetsHost(sourceText || '')
          : verificationParagraphsHost(sourceText || '', sourceUnitLengths)
        const nodes = graph && Array.isArray(graph.nodes) ? graph.nodes : []
        ...
        if (target && target.kind === 'node' && nodeById.has(target.id)) {
          ...
          for (const edge of Array.isArray(graph?.edges) ? graph.edges : []) {
            if (pSet.size >= 48) break
            const neighborId = edge?.fromNodeId === target.id ? edge.toNodeId : edge?.toNodeId === target.id ? edge.fromNodeId : null
            const neighbor = neighborId ? nodeById.get(neighborId) : null
            if (Number.isInteger(neighbor?.paragraph) && neighbor.paragraph >= 0 && neighbor.paragraph < paras.length) pSet.add(neighbor.paragraph)
          }
          ...
          const namedIds = new Set(Array.from(String(question).matchAll(/\b(?:n|m)\d+\b/gi), match => match[0]).slice(0, 16))
          const relatedIds = new Set(namedIds)
          for (const edge of Array.isArray(graph?.edges) ? graph.edges : []) {
            if (relatedIds.size >= 48) break
            if (namedIds.has(edge?.fromNodeId)) relatedIds.add(edge.toNodeId)
            if (namedIds.has(edge?.toNodeId)) relatedIds.add(edge.fromNodeId)
          }
          for (const id of relatedIds) {
            const node = nodeById.get(id)
            if (Number.isInteger(node?.paragraph) && node.paragraph >= 0 && node.paragraph < paras.length) references.push(node.paragraph)
          }
      ...
      function normalizeQuestionResult(obj, graph, sourceText, totalParagraphs, warnings, sourceUnitLengths) {
        ...
        let proposedFix = sanitizeFix(obj && obj.proposedFix, graph, sourceText, totalParagraphs, sourceUnitLengths)
      ...
      function normalizeIssueReviewResultHost(obj, task, context) {
        ...
        let proposedFix = verdict === 'confirmed'
          ? sanitizeFix(obj?.proposedFix, task.graph, task.text, context.paras.length, task.sourceUnitLengths) : { action: 'none' }
      ...
      async function runQuestionTask(task) {
        ...
        try {
          ...
          const wholeGraphReview = review?.targetKind === 'graph'
          const ctx2 = buildQuestionContext(task.graph, task.text, task.target, task.question, task.paragraphMap, task.sourceUnitLengths)
          if (review) {
            if (wholeGraphReview) {
              const nodes = task.graph.nodes || [], edges = task.graph.edges || []
              const known = new Set(nodes.map(node => node?.id).filter(Boolean))
              if (task.graph.view?.truncated || nodes.length > 96 || edges.length > 512
                || edges.some(edge => !known.has(edge?.fromNodeId) || !known.has(edge?.toNodeId))) {
                return failTask(task, 'review_context_incomplete',
                  '整图问题必须核对完整知识图，当前图不完整或超过单次 AI 复核上限；请将问题缩小到具体节点或关系，未调用模型')
              }
              // Whole-graph verdicts cannot be inferred from keyword hits or
              // clipped node fields, even when the allegation names one node.
              ctx2.pSet = new Set(ctx2.paras.map((_, index) => index))
              ctx2.sub = { summary: task.graph.summary || '', nodes, edges }
            }
            ...
            const seeds = new Set(wholeGraphReview ? [] : (review.title + ' ' + review.detail + ' ' + task.question).match(/\b(?:n|m)\d+\b/gi) || [])
            if (seeds.size > 24) return failTask(task, 'review_context_incomplete',
              '该问题引用的对照节点超过单次 AI 复核上限；请拆分问题后重试')
            if (!wholeGraphReview && task.target?.kind === 'node') seeds.add(task.target.id)
            if (!wholeGraphReview && task.target?.kind === 'edge') for (const id of task.target.id.split('>')) seeds.add(id)
            if (seeds.size > 0) {
              const nodes = task.graph.nodes || [], edges = task.graph.edges || []
              const known = new Set(nodes.map(node => node.id))
              const incident = edges.filter(edge => seeds.has(edge.fromNodeId) || seeds.has(edge.toNodeId))
              const ids = new Set(seeds)
              for (const edge of incident) { ids.add(edge.fromNodeId); ids.add(edge.toNodeId) }
              if ([...ids].some(id => !known.has(id))) return failTask(task, 'review_context_incomplete',
                '复核所需的目标、对照节点或关系端点未完整载入；请载入相关子图后重试')
              // Keep named comparisons and induced neighbor relations, not only
              // the target's star. Never silently trim evidence to fit a budget.
              const contextEdges = edges.filter(edge => ids.has(edge.fromNodeId) && ids.has(edge.toNodeId))
              const targetIncidentCount = task.target?.kind === 'node'
                ? incident.filter(edge => edge.fromNodeId === task.target.id || edge.toNodeId === task.target.id).length : 0
              if (ids.size > 96 || contextEdges.length > 512 || targetIncidentCount > 95) {
                return failTask(task, 'review_context_incomplete',
                  '目标与对照节点的完整上下文超过单次 AI 复核上限；请拆分问题或缩小目标范围后重试')
              }
              // Display previews can omit the very qualification under review.
              // Preserve complete claims and relation evidence; bound the whole
              // serialized request below rather than changing their meaning.
              ctx2.sub.nodes = nodes.filter(node => ids.has(node.id))
              ctx2.sub.edges = contextEdges
              for (const node of nodes) if (ids.has(node.id) && Number.isInteger(node.paragraph)
                && node.paragraph >= 0 && node.paragraph < ctx2.paras.length) ctx2.pSet.add(node.paragraph)
            }
            const contextIds = new Set(ctx2.sub.nodes.map(node => node.id))
            const required = reviewSourceParagraphsHost({
              nodes: task.graph.nodes.filter(node => contextIds.has(node.id)),
              edges: (task.graph.edges || []).filter(edge => contextIds.has(edge.fromNodeId) && contextIds.has(edge.toNodeId)),
            })
            for (const paragraph of required) ctx2.pSet.add(paragraph)
          }
          const units = []
          ...
          if (review && (units.length > MAX_VERIFY_SCOPE_UNITS
            || units.reduce((sum, unit) => sum + unit.text.length, 0) > MAX_VERIFY_SCOPE_CHARS)) {
            return failTask(task, 'review_source_incomplete', '复核原文上下文超过单次上限；请拆分该问题后重试，未调用模型')
          }
          ...
          if (review && userText.length > MAX_VERIFY_SCOPE_CHARS) {
            return failTask(task, 'review_context_incomplete',
              '完整复核上下文（节点、关系及原文）超过单次 AI 上限；请拆分该问题或缩小目标范围，未调用模型')
          }
          let norm = null
          let lastErr = ''
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              ...
              const r = review ? normalizeIssueReviewResultHost(obj, task, ctx2)
                : normalizeQuestionResult(obj, task.graph, task.text, ctx2.paras.length, [], task.sourceUnitLengths)
          ...
          if (review && norm.verdict === 'confirmed') {
            ...
            if (repairStatus === 'not_generated') {
              ...
              try {
                ...
                if (repairInput.length > MAX_VERIFY_SCOPE_CHARS) {
                  repairStatus = 'context_limit'
                } else {
                  const raw = await callModel(model, issueRepairPromptFor(task.graph), repairInput, 180000)
                  const obj = raw && typeof raw === 'object' ? raw : parseJson(raw)
                  const fix = sanitizeFix(obj?.proposedFix, task.graph, task.text, ctx2.paras.length, task.sourceUnitLengths)
      ...
      function buildExternalClaims(graph, sourceText, maxClaims, sourceUnitLengths) {
        ...
        const paras = verificationParagraphsHost(sourceText || '', sourceUnitLengths)
        const claims = []
      ...
      async function runFactCheckTask(task) {
        ...
        try {
          ...
          if (model) announceModel(task, model)
          const claims = buildExternalClaims(task.graph, task.text, 60, task.sourceUnitLengths)
      ...
      harness.handle('candidate-list', async (args) => {
       ...
       harness.handle('graph-commit', async (args) => {
         ...
         const ontologyConflicts = newlyInvalidOntologyEdgesHost(current.graph, merged)
         if (ontologyConflicts.length > 0) {
           return { error: { code: 'ontology_relation_conflict',
       ...
       harness.handle('extract', async (args) => {
        ...
        const checkpoint = a.checkpoint && typeof a.checkpoint === 'object' ? a.checkpoint : null
         if (checkpoint?.postprocess || checkpoint?.relationWeave) return { error: { code: 'checkpoint_invalid', message: '后处理记录只能由 Host 按 runId 从可信存储恢复，不能接纳客户端提供的候选或审校结论' } }
      ...
      harness.handle('verify-graph', async (args) => {
        ...
        const task = {
          ...
          text, graph, mode, model, paragraphMap: input.paragraphMap, sourceUnitLengths: input.sourceUnitLengths, scope: input.scoped ? { kind: 'source-units', ids: input.paragraphMap.slice() } : { kind: 'full', ids: [] }, createdAt: Date.now(),
        }
