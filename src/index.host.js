  ...
  return {
    ...
    apply(ctx) {
      ...
      const SYSTEM_PROMPT = [
        ...
        '10. 同一概念、同一事实只建一个节点；节点 text 要精炼，不要整段照抄原文。优先识别能够汇聚其他节点的核心概念、主结论或方法框架，避免把全文拆成一组同层级的孤立句子。',
        '11. 当前批次内应优先形成“核心概念/主结论 → 事实、推论、规则、例子、边界”的主干结构；如果提示附带“已有节点候选清单”，应在语义依据充分时把新节点接入其中真实存在的节点，但不得编造 id。',
        '12. 每条边的方向必须符合语义（例子/反例→被支撑项，定义→被定义项，事实→推论，因→果）。关系证据可由一个或多个 evidence 摘录共同构成；跨段关系要列出所有必要的 [P数字] 原文，端点同时出现或措辞相似不能单独证明关系。',
        '13. 与主题有关的节点应尽量接入知识主干；但原文没有充分依据时允许保持孤立，禁止为了提高连通率强行连边。',
        '14. 输出前自查：先想“这段原文真的支持这个节点/这条边吗？类型对吗？方向对吗？整张图是否有清晰主干？”，确认后再输出 JSON。',
      ].join(NL)
      ...
      const APPEND_SYSTEM_PROMPT = [
        ...
        '8. 宁缺毋滥：与已有图重复、无关的内容不要输出节点。优先让新增节点接入既有核心概念、主结论或方法框架，不要形成与旧图脱节的新孤岛。',
        '9. 输出前自查：每个新节点的 text 都能从新正文 quote 推出；关系证据可由一个或多个 evidence 原文摘录共同构成，跨段关系要列出所有必要摘录，不能仅由端点证据或名称相似强行合成；与已有节点的边必须有充分语义依据且方向正确。',
        '10. 原文没有充分依据时允许节点保持孤立，禁止为了提高连通率虚构关系。',
      ].join(NL)
      ...
      const TRAJ_APPEND_SYSTEM_PROMPT = [
        ...
        '7. 节点 id 用 n1、n2、n3... 且不得与节点清单中的已有 id 重复；单批新节点不超过 30 个。',
        '8. 宁缺毋滥：与已有图重复、无关的事件不要输出节点；优先保留「查到了什么」和「因此做出了什么判断」，并让新增节点接入已有判断主干。',
        '9. 输出前自查：每个新节点的 text 都能从新轨迹 quote 推出；关系证据可由一个或多个 evidence 轨迹摘录共同构成，跨事件关系要列出所有必要摘录，不能仅由端点证据或名称相似强行合成；与已有节点的边必须有充分语义依据且方向正确。',
        '10. 轨迹没有充分依据时允许节点保持孤立，禁止为了提高连通率虚构关系。',
      ].join(NL)
      ...
      const RELATION_WEAVE_SYSTEM_PROMPT = [
        '你是「知识图关系编织引擎」。你会收到一组已经通过验收的知识节点、已有关系，以及这些节点对应的编号原文。你的任务只是在原文证据充分时补充遗漏的关系边，使知识主干更完整。',
        '',
        '允许的关系：supports 支持 / example 例子 / counter_example 反例 / defines 定义 / infers 推断 / causes 因果',
        '',
        '硬性要求：',
        '1. 只输出关系边，禁止新增、删除、合并或改写节点。fromNodeId/toNodeId 必须来自给定节点清单。',
        '2. 只补充“已有关系”中不存在的边；禁止自环，禁止仅因关键词相似、主题相近或两个端点分别出现就连边。',
        '3. 每条边必须给出 evidence 数组。证据可以包含一个或多个编号原文摘录；同一段原文同时表达两个节点及其逻辑关系时，可以把该段摘录作为关系证据；跨段关系则列出共同证明关系所需的全部摘录。quote 必须逐字来自给定原文。',
        '4. 方向必须正确：例子/反例→被支撑项，定义→被定义项，事实→推论，因→果。example/counter_example/defines 的源节点类型必须分别是 example/counter_example/definition；supports 可表达某个事实、规则或推论为另一节点提供依据。',
        '5. 必须逐一检查输入中的“重点候选关系对”，尤其是孤立节点、低连接节点与主结论/核心概念之间的候选。候选对只是召回提示，最终仍由原文决定关系和方向。',
        '6. 原文若明确出现“导致、因此、必须由、如果…就、例子、定义、在此基础上、换言之”等逻辑信号，应补出对应关系，不要因为过度保守而省略明显关系；原文依据不足时才保持孤立。',
        '7. 每次最多补充 24 条高置信关系，宁缺毋滥，但不要在存在明确逻辑关系时返回空数组。',
        '8. 只输出合法 JSON，禁止 markdown 或解释文字。结构固定为：{"edges":[{"fromNodeId":"n1","toNodeId":"n2","relation":"supports","evidence":[{"paragraph":2,"quote":"原文逐字摘录"}]}]}',
      ].join(NL)
      ...
      function seedExplicitRelationEdgesHost(acc, paragraphTexts) {
        ...
        let added = 0
        const evidenceFor = (from, to) => {
          let evidence = mergeEvidenceRecordsHost(from && from.evidence, to && to.evidence, 8)
          if (evidence.length === 0) {
            const fallback = []
            for (const node of [from, to]) {
              if (!node || !Number.isInteger(node.paragraph) || typeof node.quote !== 'string' || !node.quote.trim()) continue
              const item = evidenceRecordHost(node.paragraph, node.quote.trim(), node)
              if (item) fallback.push(item)
            }
            evidence = mergeEvidenceRecordsHost(fallback, [], 8)
          }
          return evidence
        }
        const add = (from, to, relation) => {
          ...
          const evidence = evidenceFor(from, to)
          if (evidence.length === 0) return false
          acc.edges.push({ fromNodeId: from.id, toNodeId: to.id, relation, evidence })
        ...
        const propositionTypes = new Set(['fact', 'inference', 'rule', 'definition'])
        // Explicit concept mentions: a grounded proposition containing the
        // concept text/quote is a conservative, source-backed support link.
        for (const concept of nodes) {
          if (concept.type !== 'concept') continue
          const needles = [concept.text, concept.quote]
            .map((value) => normalizeGraphLookupTextHost(value))
            .filter((value) => value.length >= 4)
          if (needles.length === 0) continue
          for (const proposition of nodes) {
            if (!propositionTypes.has(proposition.type) || proposition.id === concept.id) continue
            const pp = paragraphOf(proposition)
            const cp = paragraphOf(concept)
            if (pp == null || cp == null || Math.abs(pp - cp) > 3) continue
            const haystack = normalizeGraphLookupTextHost(String(proposition.text || '') + ' ' + String(proposition.quote || ''))
            if (!needles.some((needle) => haystack.includes(needle))) continue
            add(proposition, concept, proposition.type === 'definition' ? 'defines' : 'supports')
          }
        }
        const inferenceMarkers = /(?:导致|以至于|因此|所以|从而|意味着|一旦|如果|只要|才会|就会)/
        // Same-paragraph proposition pairs with explicit discourse markers.
        for (let i = 0; i < nodes.length; i++) {
          for (let j = i + 1; j < nodes.length; j++) {
            ...
            if (ao >= 0 && bo >= 0) {
              ...
              const between = paragraphText.slice(start, end)
              if (((first.type === 'fact' && second.type === 'inference') || (first.type === 'rule' && second.type === 'inference')) && inferenceMarkers.test(between)) {
                linked = add(first, second, first.type === 'rule' ? 'causes' : 'infers')
              }
            }
            if (!linked && a.type === 'inference' && b.type === 'rule') add(a, b, 'supports')
            else if (!linked && b.type === 'inference' && a.type === 'rule') add(b, a, 'supports')
          }
        }
        ...
        for (const node of nodes) {
          ...
          if (!nodesByParagraph.has(paragraph)) nodesByParagraph.set(paragraph, [])
          nodesByParagraph.get(paragraph).push(node)
        }
        const priority = (node) => node.type === 'inference' ? 0 : node.type === 'rule' ? 1 : node.type === 'fact' ? 2 : node.type === 'example' ? 3 : 4
        const transitions = /^(?:\s|[“"'（(])*?(?:因此|所以|为此|在此基础上|换言之|正因如此|这正是)/
        for (let paragraph = 1; paragraph < paragraphTexts.length && added < 16; paragraph++) {
          if (!transitions.test(String(paragraphTexts[paragraph] || ''))) continue
          const current = (nodesByParagraph.get(paragraph) || []).filter((node) => propositionTypes.has(node.type)).sort((a, b) => priority(a) - priority(b) || offsetOf(a) - offsetOf(b))
          if (current.length === 0) continue
          let previous = []
          for (let distance = 1; distance <= 2 && previous.length === 0; distance++) previous = nodesByParagraph.get(paragraph - distance) || []
          previous = previous.slice().sort((a, b) => (b.type === 'example' ? 1 : 0) - (a.type === 'example' ? 1 : 0) || priority(a) - priority(b) || offsetOf(b) - offsetOf(a))
          if (previous.length > 0) add(previous[0], current[0], previous[0].type === 'example' ? 'example' : 'supports')
        }
        // Example nodes often precede the conclusion that names what the
        // example demonstrates. Keep this bounded to the next two paragraphs
        // and require an explicit example/analogy discourse signal.
        for (const example of nodes.filter((node) => node.type === 'example')) {
          const paragraph = paragraphOf(example)
          if (paragraph == null) continue
          for (let targetParagraph = paragraph + 1; targetParagraph <= Math.min(paragraph + 2, paragraphTexts.length - 1); targetParagraph++) {
            const span = paragraphTexts.slice(paragraph, targetParagraph + 1).join(' ')
            if (!/(?:例如|比如|就好比|这正是|如果是|这可能)/.test(span)) continue
            const targets = (nodesByParagraph.get(targetParagraph) || []).filter((node) => propositionTypes.has(node.type)).sort((a, b) => priority(a) - priority(b) || offsetOf(a) - offsetOf(b))
            if (targets.length > 0 && add(example, targets[0], 'example')) break
          }
        }
        return added
      }
