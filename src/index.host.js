    ...
    apply(ctx) {
      ...
      async function weaveRelationsHost(task, model, acc, paragraphTexts, sourceInfo, sourceText, previousCoverage = null) {
        ...
        const binding = sha256HexHost(JSON.stringify({
          policy: 'relation-weave-v1', ontology: ontIdOf(task), sourceText,
          base: invariantRepairSnapshotHost({ nodes, edges: acc.edges }),
          groups: groups.map(group => ({ ids: group.nodes.map(node => node.id), targets: group.targetIds, context: group.sharedContext })),
        }))
        let journal = task.relationWeave || { version: 1, binding, totalGroups: groups.length, results: {} }
        const invalidJournal = () => taskOperationErrorHost('checkpoint_invalid', '关系补全检查点与原文、基础图或分组计划不一致，未重复调用模型', 'relation_weave')
        if (journal.version !== 1 || journal.binding !== binding || journal.totalGroups !== groups.length || !journal.results || typeof journal.results !== 'object' || Array.isArray(journal.results)) throw invalidJournal()
